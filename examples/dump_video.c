...
static void open_video(void)
{
	...
	for (pli = 0; pli < 3; pli++)
	{
		...
		yshift = pli != 0 && !(ti.pixel_fmt & 2);
		ycbcr[pli].data =
			(unsigned char*)malloc((ti.frame_width >> xshift) * (ti.frame_height >> yshift) * sizeof(*ycbcr[pli].data));
...
static void video_write(void)
{
	...
	if (outfile)
	{
		...
		for (pli = 0; pli < 3; pli++)
		{
			...
			{
				fwrite(ycbcr[pli].data + ycbcr[pli].stride * i + (x0 >> hdec), 1, (xend + hdec >> hdec) - (x0 >> hdec),
					   outfile);
			}
			hdec = !(ti.pixel_fmt & 1);
			vdec = !(ti.pixel_fmt & 2);
...
{
	fprintf(stderr, "Usage: dumpvid [options] [<infile.ogv>] [-o <outfile.y4m>]\n\n"
...
int main(int argc, char* argv[])
{
	...
	if (!raw && outfile)
	{
		...
		if (crop)
		{
			...
			hdec = !(ti.pixel_fmt & 1);
			vdec = !(ti.pixel_fmt & 2);
			if ((ti.pic_x & hdec) || (ti.pic_width & hdec) || (ti.pic_y & vdec) || (ti.pic_height & vdec))
			{
				fprintf(stderr, "Error: Cropped images with odd offsets/sizes and chroma subsampling\n"
		...
		fprintf(outfile, "YUV4MPEG2 C%s W%d H%d F%d:%d I%c A%d:%d\n", CHROMA_TYPES[ti.pixel_fmt], width, height,
				ti.fps_numerator, ti.fps_denominator, 'p', ti.aspect_numerator, ti.aspect_denominator);
	}
	...
	while (!got_sigint)
	{
		...
		{
			long ms = after.time * 1000. + after.millitm - (last.time * 1000. + last.millitm);

			if (ms > 500 || fps_only == 1 || (feof(infile) && !videobuf_ready))
			{
				float file_fps = (float)ti.fps_numerator / ti.fps_denominator;
				fps_only = 2;

				ms = after.time * 1000. + after.millitm - (start.time * 1000. + start.millitm);

				fprintf(stderr, "\rframe:%d rate:%.2fx           ", frames, frames * 1000. / (ms * file_fps));
