...
#endif
	;
struct option options[] = {{"begin-time", required_argument, NULL, 'b'},
...
{
	fprintf(stderr, "Usage: encoder_example [options] [audio_file] video_file\n\n"
					"Options: \n\n"
					"  -o --output <filename.ogv>      file name for encoded output;\n"
					"                                  If this option is not given, the\n"
					"                                  compressed data is sent to stdout.\n\n"
					"  -A --audio-rate-target <n>      bitrate target for Vorbis audio;\n"
					"                                  use -a and not -A if at all "
					"possible,\n"
					"                                  as -a gives higher quality for a "
					"given\n"
					"                                  bitrate.\n\n"
					"  -V --video-rate-target <n>      bitrate target for Theora video\n\n"
					"     --soft-target                Use a large reservoir and treat the "
					"rate\n"
					"                                  as a soft target; rate control is "
					"less\n"
					"                                  strict but resulting quality is "
					"usually\n"
					"                                  higher/smoother overall. Soft target "
					"also\n"
					"                                  allows an optional -v setting to "
					"specify\n"
					"                                  a minimum allowed quality.\n\n"
					"     --two-pass                   Compress input using two-pass rate "
					"control\n"
					"                                  This option requires that the input "
					"to the\n"
					"                                  to the encoder is seekable and "
					"performs\n"
					"                                  both passes automatically.\n\n"
					"     --first-pass <filename>      Perform first-pass of a two-pass "
					"rate\n"
					"                                  controlled encoding, saving pass data "
					"to\n"
					"                                  <filename> for a later second pass\n\n"
					"     --second-pass <filename>     Perform second-pass of a two-pass "
					"rate\n"
					"                                  controlled encoding, reading "
					"first-pass\n"
					"                                  data from <filename>.  The first "
					"pass\n"
					"                                  data must come from a first encoding "
					"pass\n"
					"                                  using identical input video to work\n"
					"                                  properly.\n\n"
					"  -a --audio-quality <n>          Vorbis quality selector from -1 to "
					"10\n"
					...
					"   -S --aspect-denominator <n>    Aspect ratio denominator, default is "
					"0\n"
					"                                  or extracted from YUV input file\n"
					"   -f --framerate-numerator <n>   Frame rate numerator, can be "
					"extracted\n"
					"                                  from YUV input file. ex: 30000000\n"
					"   -F --framerate-denominator <n> Frame rate denominator, can be "
					"extracted\n"
					"                                  from YUV input file. ex: 1000000\n"
					"                                  The frame rate nominator divided by "
					"this\n"
					"                                  determinates the frame rate in units "
					"per tick\n"
					"   -k --keyframe-freq <n>         Keyframe frequency\n"
					"   -z --speed <n>                 Sets the encoder speed level. Higher "
					"speed\n"
					"                                  levels favor quicker encoding over "
					"better\n"
					"                                  quality per bit. Depending on the "
					"encoding\n"
					"                                  mode, and the internal algorithms "
					"used,\n"
					"                                  quality may actually improve with "
					"higher\n"
					"                                  speeds, but in this case bitrate will "
					"also\n"
					"                                  likely increase. The maximum value, "
					"and the\n"
					"                                  meaning of each value, are "
					"implementation-\n"
					"                                  specific and may change depending on "
					"the\n"
					"                                  current encoding mode (rate "
					"constrained,\n"
					"                                  two-pass, etc.).\n"
					"   -d --buf-delay <n>             Buffer delay (in frames). Longer "
					"delays\n"
					"                                  allow smoother rate adaptation and "
					"provide\n"
					"                                  better overall quality, but require "
					"more\n"
					"                                  client side buffering and add "
					"latency. The\n"
					"                                  default value is the keyframe "
					"interval for\n"
					"                                  one-pass encoding (or somewhat larger "
					"if\n"
					"                                  --soft-target is used) and infinite "
					"for\n"
					"                                  two-pass encoding.\n"
...
#if defined(OC_COLLECT_METRICS)
					"   -m --metrics-filename          File in which to accumulate mode "
					"decision\n"
					"                                  metrics. Statistics from the current\n"
					"                                  encode will be merged with those "
					"already\n"
...
/*All anti-aliasing filters in the following conversion functions are based on one of two window functions:
  ...
  The 4-tap Mitchell window (for up-sampling):
   ...
   0,                               |t|>=2
  The number of taps is intentionally kept small to reduce computational overhead and limit ringing.

  The taps from these filters are scaled so that their sum is 1, and the result is scaled by 128 and rounded to integers
  to create a filter whose intermediate values fit inside 16 bits.
  Coefficients are rounded in such a way as to ensure their sum is still 128, which is usually equivalent to normal
  rounding.*/
...
static void y4m_convert_42xmpeg2_42xjpeg(unsigned char* _dst, unsigned char* _aux)
{
	int c_w;
	int c_h;
	int pli;
		 ...
		 window.*/
			for (x = 0; x < OC_MINI(c_w, 2); x++)
			{
				_dst[x] = (unsigned char)OC_CLAMPI(
					0, 4 * _aux[0] - 17 * _aux[OC_MAXI(x - 1, 0)] + 114 * _aux[x] + 35 * _aux[OC_MINI(x + 1, c_w - 1)] -
							   9 * _aux[OC_MINI(x + 2, c_w - 1)] + _aux[OC_MINI(x + 3, c_w - 1)] + 64 >>
						   7,
					255);
			}
			for (; x < c_w - 3; x++)
			{
				_dst[x] = (unsigned char)OC_CLAMPI(0, 4 * _aux[x - 2] - 17 * _aux[x - 1] + 114 * _aux[x] +
															  35 * _aux[x + 1] - 9 * _aux[x + 2] + _aux[x + 3] + 64 >>
														  7,
												   255);
			}
			for (; x < c_w; x++)
			{
				_dst[x] = (unsigned char)OC_CLAMPI(0, 4 * _aux[x - 2] - 17 * _aux[x - 1] + 114 * _aux[x] +
															  35 * _aux[OC_MINI(x + 1, c_w - 1)] -
															  9 * _aux[OC_MINI(x + 2, c_w - 1)] + _aux[c_w - 1] + 64 >>
														  7,
												   255);
			}
			_dst += c_w;
...
static void y4m_convert_42xpaldv_42xjpeg(unsigned char* _dst, unsigned char* _aux)
{
	unsigned char* tmp;
	int c_w;
	int c_h;
	...
	for (pli = 1; pli < 3; pli++)
	{
		for (y = 0; y < c_h; y++)
		{
		 ...
		 window.*/
			...
			for (; x < c_w - 3; x++)
			{
				tmp[x] = (unsigned char)OC_CLAMPI(0, 4 * _aux[x - 2] - 17 * _aux[x - 1] + 114 * _aux[x] +
															 35 * _aux[x + 1] - 9 * _aux[x + 2] + _aux[x + 3] + 64 >>
														 7,
												  255);
			}
				...
				for (x = 0; x < c_w; x++)
				{
					for (y = 0; y < OC_MINI(c_h, 3); y++)
					{
						_dst[y * c_w] = (unsigned char)OC_CLAMPI(
							0, tmp[0] - 9 * tmp[OC_MAXI(y - 2, 0) * c_w] + 35 * tmp[OC_MAXI(y - 1, 0) * c_w] +
					...
					{
						_dst[y * c_w] = (unsigned char)OC_CLAMPI(
							0, tmp[(y - 3) * c_w] - 9 * tmp[(y - 2) * c_w] + 35 * tmp[(y - 1) * c_w] +
					...
					for (; y < c_h; y++)
					{
						_dst[y * c_w] = (unsigned char)OC_CLAMPI(
							0,
							tmp[(y - 3) * c_w] - 9 * tmp[(y - 2) * c_w] + 35 * tmp[(y - 1) * c_w] + 114 * tmp[y * c_w] -
									17 * tmp[OC_MINI(y + 1, c_h - 1) * c_w] + 4 * tmp[(c_h - 1) * c_w] + 64 >>
								7,
							255);
					}
					_dst++;
			...
			case 2:
			{
			  ...
			  This is the same as the horizontal filter.*/
				for (x = 0; x < c_w; x++)
				{
					...
					{
						_dst[y * c_w] = (unsigned char)OC_CLAMPI(0, 4 * tmp[0] - 17 * tmp[OC_MAXI(y - 1, 0) * c_w] +
																			114 * tmp[y * c_w] +
																			35 * tmp[OC_MINI(y + 1, c_h - 1) * c_w] -
					...
					{
						_dst[y * c_w] = (unsigned char)OC_CLAMPI(
							0, 4 * tmp[(y - 2) * c_w] - 17 * tmp[(y - 1) * c_w] + 114 * tmp[y * c_w] +
					...
					for (; y < c_h; y++)
					{
						_dst[y * c_w] = (unsigned char)OC_CLAMPI(
							0, 4 * tmp[(y - 2) * c_w] - 17 * tmp[(y - 1) * c_w] + 114 * tmp[y * c_w] +
									   35 * tmp[OC_MINI(y + 1, c_h - 1) * c_w] -
									   9 * tmp[OC_MINI(y + 2, c_h - 1) * c_w] + tmp[(c_h - 1) * c_w] + 64 >>
								   7,
							255);
					}
					_dst++;
...
/*422jpeg chroma samples are sited like:
  ...
  We use a filter to resample at site locations one eighth pixel (at the source
   ...
   right.*/
static void y4m_convert_411_422jpeg(unsigned char* _dst, unsigned char* _aux)
{
	int c_w;
	int dst_c_w;
	int c_h;
		 ...
		 4-tap Mitchell window.*/
			for (x = 0; x < OC_MINI(c_w, 1); x++)
			{
				_dst[x << 1] = (unsigned char)OC_CLAMPI(
					0, 111 * _aux[0] + 18 * _aux[OC_MINI(1, c_w - 1)] - _aux[OC_MINI(2, c_w - 1)] + 64 >> 7, 255);
				_dst[x << 1 | 1] = (unsigned char)OC_CLAMPI(
					0, 47 * _aux[0] + 86 * _aux[OC_MINI(1, c_w - 1)] - 5 * _aux[OC_MINI(2, c_w - 1)] + 64 >> 7, 255);
			}
			for (; x < c_w - 2; x++)
			{
				_dst[x << 1] = (unsigned char)OC_CLAMPI(
					0, _aux[x - 1] + 110 * _aux[x] + 18 * _aux[x + 1] - _aux[x + 2] + 64 >> 7, 255);
				_dst[x << 1 | 1] = (unsigned char)OC_CLAMPI(
					0, -3 * _aux[x - 1] + 50 * _aux[x] + 86 * _aux[x + 1] - 5 * _aux[x + 2] + 64 >> 7, 255);
			}
			for (; x < c_w; x++)
			{
				_dst[x << 1] = (unsigned char)OC_CLAMPI(
					0, _aux[x - 1] + 110 * _aux[x] + 18 * _aux[OC_MINI(x + 1, c_w - 1)] - _aux[c_w - 1] + 64 >> 7, 255);
				if ((x << 1 | 1) < dst_c_w)
				{
					_dst[x << 1 | 1] = (unsigned char)OC_CLAMPI(
						0,
						-3 * _aux[x - 1] + 50 * _aux[x] + 86 * _aux[OC_MINI(x + 1, c_w - 1)] - 5 * _aux[c_w - 1] + 64 >>
							7,
						255);
				}
			}
...
static void y4m_convert_mono_420jpeg(unsigned char* _dst, unsigned char* _aux)
{
	int c_sz;
	_dst += pic_w * pic_h;
...
static void y4m_convert_null(unsigned char* _dst, unsigned char* _aux) {}
...
static void id_file(char* f)
{
		...
		if (!memcmp(buffer + 4, "WAVE", 4))
		{
			while (!feof(test))
			{
				...
				if (!memcmp("fmt", buffer, 3))
				{
					...
					audio_hz = buffer[8] + (buffer[9] << 8) + (buffer[10] << 16) + (buffer[11] << 24);
	...
	if (!memcmp(buffer, "YUV4", 4))
	{
		...
		if (!memcmp(buffer, "MPEG", 4))
		{
			...
			if (interlace != 'p')
			{
				fprintf(stderr, "Input video is interlaced; Theora handles only "
								"progressive scan\n");
				exit(1);
			}
		 ...
		 destination chroma decimation type.*/
			y4m_dst_buf_sz = pic_w * pic_h +
							 2 * ((pic_w + dst_c_dec_h - 1) / dst_c_dec_h) * ((pic_h + dst_c_dec_v - 1) / dst_c_dec_v);
			...
			if (!quiet)
			{
				fprintf(stderr, "File %s is %dx%d %.02f fps %s video.\n", f, pic_w, pic_h,
						(double)video_fps_n / video_fps_d, chroma_type);
			}
...
int fetch_and_process_video_packet(FILE* video, FILE* twopass_file, int passno, th_enc_ctx* td, ogg_packet* op)
{
	int ret;
	int pic_sz;
	int c_w;
...
int fetch_and_process_video(FILE* video, ogg_page* videopage, ogg_stream_state* to, th_enc_ctx* td, FILE* twopass_file,
							int passno, int videoflag)
{
	ogg_packet op;
	int ret;
...
int main(int argc, char* argv[])
{
...
#if defined(OC_COLLECT_METRICS)
			case 'm':
				if (th_encode_ctl(NULL, TH_ENCCTL_SET_METRICS_FILE, optarg, strlen(optarg) + 1))
				{
	...
	if (soft_target)
	{
		if (video_r <= 0)
		{
			fprintf(stderr, "Soft rate target (--soft-target) requested without a "
							"bitrate (-V).\n");
			exit(1);
		}
		if (video_q == -1)
	 ...
	 */
	  ...
	  This is 1 byte per 255 for lacing values, plus 26 bytes per 4096 bytes for
	   the page header, plus approximately 1/2 byte per packet (not accounted
	  for
	   here).*/
	...
	{
		fprintf(stderr, "\r   \n");
		fprintf(stderr, "      %lld frames in %.3lf seconds: %.3lf Mpixel/s", (long long)frames, elapsed,
				(double)1e-6 * frames * frame_w * frame_h / elapsed);
		fprintf(stderr, " %.2lfx", (double)frames * video_fps_d / (elapsed * video_fps_n));
		fprintf(stderr, "\ndone.\n\n");
	}
