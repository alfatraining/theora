...
struct option options[] = {{"frame-type", no_argument, NULL, 'f'},
...
typedef void (*y4m_convert_func)(y4m_input* _y4m, unsigned char* _dst, unsigned char* _aux);
...
/*422jpeg chroma samples are sited like:
  ...
  We use a filter to resample at site locations one eighth pixel (at the source
   ...
   right.*/
static void y4m_convert_411_422jpeg(y4m_input* _y4m, unsigned char* _dst, unsigned char* _aux)
{
	int c_w;
	int dst_c_w;
	int c_h;
...
static void y4m_convert_null(y4m_input* _y4m, unsigned char* _dst, unsigned char* _aux) {}
...
static void y4m_input_get_info(y4m_input* _y4m, th_info* _ti)
{
	...
	_ti->pixel_fmt = _y4m->dst_c_dec_h == 2 ? (_y4m->dst_c_dec_v == 2 ? TH_PF_420 : TH_PF_422) : TH_PF_444;
}

static int y4m_input_fetch_frame(y4m_input* _y4m, FILE* _fin, th_ycbcr_buffer _ycbcr)
{
	char frame[6];
	int pic_sz;
	int frame_c_w;
	...
	_ycbcr[1].stride = c_w;
	_ycbcr[1].data =
		_y4m->dst_buf + pic_sz - (_y4m->pic_x / _y4m->dst_c_dec_h) - (_y4m->pic_y / _y4m->dst_c_dec_v) * c_w;
...
{
	if (_th->theora_p)
		ogg_stream_pagein(&_th->to, _og);
}

static int th_input_open_impl(th_input* _th, th_setup_info** _ts, FILE* _fin, char* _sig, int _nsig)
{
	ogg_packet op;
	ogg_page og;
	int nheaders_left;
	...
	*_ts = NULL;
	...
	{
		if (th_input_buffer_data(_th, _fin) == 0)
			break;
		while (ogg_sync_pageout(&_th->oy, &og) > 0)
		{
			...
			if (!_th->theora_p)
			{
				...
				if (nheaders_left >= 0)
				{
					...
					if (nheaders_left > 0)
						ogg_stream_packetout(&_th->to, NULL);
					continue;
				}
			}
	...
	if (_th->theora_p)
	{
		...
		if (_th->td != NULL)
		{
			fprintf(stderr, "Ogg logical stream %lx is Theora %ix%i %.02f fps video.\n"
					...
					(double)_th->ti.fps_numerator / _th->ti.fps_denominator, _th->ti.pic_width, _th->ti.pic_height,
					_th->ti.pic_x, _th->ti.pic_y);
			return 1;
		}
	}
	return -1;
...
static void th_input_get_info(th_input* _th, th_info* _ti) { memcpy(_ti, &_th->ti, sizeof(*_ti)); }

static int th_input_fetch_frame(th_input* _th, FILE* _fin, th_ycbcr_buffer _ycbcr)
{
	for (;;)
	{
		ogg_page og;
...
typedef int (*video_input_fetch_frame_func)(void* _ctx, FILE* _fin, th_ycbcr_buffer _ycbcr);
...
static int video_input_open(video_input* _vid, FILE* _fin)
{
	...
	if (ret < 4)
		fprintf(stderr, "EOF determining file type of file.\n");
	else
	{
...
static void video_input_get_info(video_input* _vid, th_info* _ti) { (*_vid->get_info)(&_vid->ctx, _ti); }
...
int main(int _argc, char* _argv[])
{
	...
	if (ti1.fps_numerator * (ogg_int64_t)ti2.fps_denominator != ti2.fps_numerator * (ogg_int64_t)ti1.fps_denominator)
	{
	...
	for (frameno = 0;; frameno++)
	{
		...
		for (pli = 0; pli < 3; pli++)
		{
			...
			plsqerr[pli] = 0;
			plnpixels[pli] = 0;
			for (y1 = ti1.pic_y >> ydec, y2 = ti2.pic_y >> ydec; y1<ti1.pic_y + ti1.pic_height + ydec>> ydec;
				 y1++, y2++)
			{
				int x1;
				int x2;
				for (x1 = ti1.pic_x >> xdec, x2 = ti2.pic_x >> xdec; x1<ti1.pic_x + ti1.pic_width + xdec>> xdec;
					 x1++, x2++)
				{
					int d;
					d = *(f1[pli].data + y1 * f1[pli].stride + x1) - *(f2[pli].data + y2 * f2[pli].stride + x2);
					plsqerr[pli] += d * d;
					plnpixels[pli]++;
				}
			}
		...
		if (!summary_only)
		{
			...
			{
				printf("%08i: %-7lG\n", frameno, 10 * (log10(255 * 255) + log10(plnpixels[0]) - log10(plsqerr[0])));
			}
		}
		gsqerr += sqerr;
		gnpixels += npixels;
