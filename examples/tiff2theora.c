...
{
	return strcoll((*(const struct dirent**)a)->d_name, (*(const struct dirent**)b)->d_name);
}
...
static void rgb_to_yuv(uint32* raster, th_ycbcr_buffer ycbcr, unsigned int w, unsigned int h)
{
	unsigned int x;
	unsigned int y;
			...
			for (x = 0; x < w; x += 2)
			{
				...
				yuv_u[(x >> 1) + (y >> 1) * ycbcr[1].stride] =
					clamp(((-33488 * r0 - 65744 * g0 + 99232 * b0 + 29032005) / 4 +
						   ...
						   (-33488 * r3 - 65744 * g3 + 99232 * b3 + 29032005) / 4) /
						  225930);
				yuv_v[(x >> 1) + (y >> 1) * ycbcr[2].stride] =
					clamp(((157024 * r0 - 131488 * g0 - 25536 * b0 + 45940035) / 4 +
						   ...
						   (157024 * r3 - 131488 * g3 - 25536 * b3 + 45940035) / 4) /
						  357510);
			}
		}
	}
	...
	{ /* TH_PF_422 */
		for (y = 0; y < h; y += 1)
		{
			for (x = 0; x < w; x += 2)
			{
				...
				yuv_u[(x >> 1) + y * ycbcr[1].stride] = clamp(((-33488 * r0 - 65744 * g0 + 99232 * b0 + 29032005) / 2 +
															   (-33488 * r1 - 65744 * g1 + 99232 * b1 + 29032005) / 2) /
															  225930);
				yuv_v[(x >> 1) + y * ycbcr[2].stride] =
					clamp(((157024 * r0 - 131488 * g0 - 25536 * b0 + 45940035) / 2 +
						   (157024 * r1 - 131488 * g1 - 25536 * b1 + 45940035) / 2) /
						  357510);
			}
		}
	}
...
static int tiff_read(const char* pathname, unsigned int* w, unsigned int* h, th_ycbcr_buffer ycbcr)
{
	TIFF* tiff;
	uint32 width;
	uint32 height;
