...
void audio_calibrate_timer(int restart)
{
	...
	if (restart)
	{
		...
		current_sample = audiobuf_granulepos - (audiobuf_fill + audiofd_totalsize - audiofd_fragsize) / 2 / vi.channels;
...
int got_sigint = 0;
static void sigint_handler(int signal) { got_sigint = 1; }
...
static void open_video(void)
{
	...
	if (px_fmt == TH_PF_422)
		yuv_overlay = SDL_CreateYUVOverlay(w, h, SDL_YUY2_OVERLAY, screen);
...
static void video_write(void)
{
	...
	if (px_fmt != TH_PF_444 && SDL_LockYUVOverlay(yuv_overlay) < 0)
		return;
	...
	else if (px_fmt == TH_PF_444)
	{
		...
		for (i = 0; i < screen->h; i++)
		{
			...
			for (j = 0; j < screen->w; j++)
			{
				...
				g = (3827562 * in_y[j] - 1287801 * in_u[j] - 2672387 * in_v[j] + 447306710) / 3287200;
	...
	else
	{
		...
		for (i = 0; i < yuv_overlay->h; i++)
			memcpy(yuv_overlay->pixels[0] + yuv_overlay->pitches[0] * i, yuv[0].data + y_offset + yuv[0].stride * i,
				   yuv_overlay->w);
		for (i = 0; i < yuv_overlay->h / 2; i++)
		{
			memcpy(yuv_overlay->pixels[1] + yuv_overlay->pitches[1] * i, yuv[2].data + uv_offset + yuv[2].stride * i,
				   yuv_overlay->w / 2);
			memcpy(yuv_overlay->pixels[2] + yuv_overlay->pitches[2] * i, yuv[1].data + uv_offset + yuv[1].stride * i,
				   yuv_overlay->w / 2);
		}
	}
...
{
	fprintf(stderr, "Usage: player_example <file.ogv>\n"
...
int main(int argc, char* const* argv)
{
	...
	if (theora_p)
	{
		...
		printf("Ogg logical stream %lx is Theora %dx%d %.02f fps", to.serialno, ti.pic_width, ti.pic_height,
		...
		if (ti.pic_width != ti.frame_width || ti.pic_height != ti.frame_height)
			printf("  Frame content is %dx%d with offset (%d,%d).\n", ti.frame_width, ti.frame_height, ti.pic_x,
				   ti.pic_y);
	...
	while (!got_sigint)
	{
		...
		if (stateflag && (audiobuf_ready || !vorbis_p) && (videobuf_ready || !theora_p) && !got_sigint)
		{
	...
	if (dropped)
		fprintf(stderr, " (%d dropped)", dropped);
	fprintf(stderr, "\n");
	fprintf(stderr, "\nDone.\n");
