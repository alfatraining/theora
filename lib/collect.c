...
void oc_mode_metrics_add(oc_mode_metrics* _metrics, double _w, int _s, int _q, int _r, double _d)
{
	if (_metrics->w > 0)
	{
		double ds;
		...
		_metrics->s2q2 +=
			-2 * (ds * sq2 + dq * s2q) * rwb + (ds2 * q2 + 4 * ds * dq * sq + dq2 * s2) * rwb2 + ds2 * dq2 * rw4;
...
void oc_mode_metrics_merge(oc_mode_metrics* _dst, const oc_mode_metrics* _src, int _n)
{
	int i;
	...
	for (i++; i < _n; i++)
		if (_src[i].w != 0)
		{
			...
			*/
			...
			_dst->s2q += _src[i].s2q + 2 * ds * (rwa * sqb - rwb * sqa) + dq * (rwa * s2b - rwb * s2a) + ds2 * dq * rw3;
			_dst->sq2 += _src[i].sq2 + ds * (rwa * q2b - rwb * q2a) + 2 * dq * (rwa * sqb - rwb * sqa) + ds * dq2 * rw3;
...
double oc_mode_metrics_solve(double* _r, double* _d, const oc_mode_metrics* _metrics, const int* _s0, const int* _s1,
							 const int* _q0, const int* _q1, const double* _ra, const double* _rb, const double* _rc,
							 const double* _da, const double* _db, const double* _dc, int _n)
{
	...
	for (i = 0; i < _n; i++)
		if (_metrics[i].w > 0)
		{
			...
			xx +=
				(dq2 * (ds2 * w + s2) + 4 * ds * dq * sq + ds2 * q2 + 2 * (dq * s2q + ds * sq2) + s2q2) / (sq10 * sq10);
			r = _metrics[i].r / w;
			sr = _metrics[i].sr;
			qr = _metrics[i].qr;
			sqr = _metrics[i].sqr;
			...
			rxy += (ds * dq * (r + c0 + c1 * ds + c2 * dq + c3 * ds * dq) * w + (c1 * dq + c3 * dq2) * s2 +
					(r + c0 + 2 * (c1 * ds + (c2 + 2 * c3 * ds) * dq)) * sq + (c2 * ds + c3 * ds2) * q2 + dq * sr +
					ds * qr + (c1 + 2 * c3 * dq) * s2q + (c2 + 2 * c3 * ds) * sq2 + sqr + c3 * s2q2) /
				   sq10;
			d = _metrics[i].d / w;
			sd = _metrics[i].sd;
			qd = _metrics[i].qd;
			...
			dxy += (ds * dq * (d + c0 + c1 * ds + c2 * dq + c3 * ds * dq) * w + (c1 * dq + c3 * dq2) * s2 +
					(d + c0 + 2 * (c1 * ds + (c2 + 2 * c3 * ds) * dq)) * sq + (c2 * ds + c3 * ds2) * q2 + dq * sd +
					ds * qd + (c1 + 2 * c3 * dq) * s2q + (c2 + 2 * c3 * ds) * sq2 + sqd + c3 * s2q2) /
				   sq10;
		}
	if (xx > 1E-3)
	{
...
void oc_mode_metrics_update(oc_mode_metrics (*_metrics)[3][2][OC_COMP_BINS], int _niters_min, int _reweight,
							oc_mode_rd (*_table)[3][2][OC_COMP_BINS], int _shift, double (*_weight)[3][2][OC_COMP_BINS])
{
	int niters;
	int prevdr;
	int prevdd;
				...
				for (qi = 0; qi < OC_LOGQ_BINS; qi++)
				{
					for (si = 0; si < OC_COMP_BINS; si++)
					{
						...
						if (qi > 0)
						{
							...
							da[n] = ldexp(_table[qi - 1][pli][qti][si + ds].rmse, -OC_RMSE_SCALE);
							rb[n] = ldexp(_table[qi - 1][pli][qti][si].rate, -OC_BIT_SCALE);
							db[n] = ldexp(_table[qi - 1][pli][qti][si].rmse, -OC_RMSE_SCALE);
			...
			for (qti = 0; qti < 2; qti++)
			{
				for (qi = 0; qi < OC_LOGQ_BINS; qi++)
				{
					for (si = 0; si < OC_COMP_BINS; si++)
					{
						...
						wt /= OC_ZWEIGHT + wt;
						_table[qi][pli][qti][si].rate = (ogg_int16_t)(_table[qi][pli][qti][si].rate * wt + 0.5);
						_table[qi][pli][qti][si].rmse = (ogg_int16_t)(_table[qi][pli][qti][si].rmse * wt + 0.5);
					}
				}
			}
		}
...
#endif
	int qii;
	fprintf(_fout, "# if !defined(OC_COLLECT_METRICS)\n"
				   "static const\n"
				   "# endif\n"
		...
		for (pli = 0; pli < 3; pli++)
		{
			...
			for (qti = 0; qti < 2; qti++)
			{
				...
				for (bin = 0; bin < OC_COMP_BINS; bin++)
				{
					if (bin && !(bin & 0x3))
						fprintf(_fout, "\n        ");
					fprintf(_fout, "{%5i,%5i}", _mode_rd_table[qii][pli][qti][bin].rate,
							_mode_rd_table[qii][pli][qti][bin].rmse);
					if (bin + 1 < OC_COMP_BINS)
						fprintf(_fout, ",");
				}
				fprintf(_fout, "\n      }");
				if (qti < 1)
...
{
	int qii;
	fprintf(_fout, "/*File generated by libtheora with OC_COLLECT_METRICS"
	...
	for (qii = 0; qii < OC_LOGQ_BINS; qii++)
	{
		fprintf(_fout, "  { {0x%04X,0x%04X},{0x%04X,0x%04X},{0x%04X,0x%04X} }%s\n", OC_MODE_LOGQ[qii][0][0],
				OC_MODE_LOGQ[qii][0][1], OC_MODE_LOGQ[qii][1][0], OC_MODE_LOGQ[qii][1][1], OC_MODE_LOGQ[qii][2][0],
				OC_MODE_LOGQ[qii][2][1], qii + 1 < OC_LOGQ_BINS ? "," : "");
	}
	fprintf(_fout, "};\n"
				   "\n");
...
void oc_enc_mode_metrics_load(oc_enc_ctx* _enc)
{
	...
	if (!OC_HAS_MODE_METRICS)
	{
		...
		oc_mode_metrics_update(OC_MODE_METRICS_SATD, 100, 1, OC_MODE_RD_SATD, OC_SATD_SHIFT, OC_MODE_RD_WEIGHT_SATD);
		oc_mode_metrics_update(OC_MODE_METRICS_SAD, 100, 1, OC_MODE_RD_SAD, OC_SAD_SHIFT, OC_MODE_RD_WEIGHT_SAD);
...
static ptrdiff_t oc_token_skip_zrl(int _token, int _extra_bits) { return _extra_bits + 1; }

/*Handles a normal coefficient value token.*/
static ptrdiff_t oc_token_skip_val(void) { return 1; }

/*Handles a category 1A zero run/coefficient value combo token.*/
static ptrdiff_t oc_token_skip_run_cat1a(int _token) { return _token - OC_DCT_RUN_CAT1A + 2; }
...
static const oc_token_skip_func OC_TOKEN_SKIP_TABLE[TH_NDCT_TOKENS] = {
	oc_token_skip_eob, oc_token_skip_eob, oc_token_skip_eob, oc_token_skip_eob, oc_token_skip_eob, oc_token_skip_eob,
	oc_token_skip_eob6, oc_token_skip_zrl, oc_token_skip_zrl, (oc_token_skip_func)oc_token_skip_val,
	(oc_token_skip_func)oc_token_skip_val, (oc_token_skip_func)oc_token_skip_val, (oc_token_skip_func)oc_token_skip_val,
	(oc_token_skip_func)oc_token_skip_val, (oc_token_skip_func)oc_token_skip_val, (oc_token_skip_func)oc_token_skip_val,
	(oc_token_skip_func)oc_token_skip_val, (oc_token_skip_func)oc_token_skip_val, (oc_token_skip_func)oc_token_skip_val,
	(oc_token_skip_func)oc_token_skip_val, (oc_token_skip_func)oc_token_skip_val, (oc_token_skip_func)oc_token_skip_val,
	(oc_token_skip_func)oc_token_skip_val, (oc_token_skip_func)oc_token_skip_run_cat1a,
	(oc_token_skip_func)oc_token_skip_run_cat1a, (oc_token_skip_func)oc_token_skip_run_cat1a,
	(oc_token_skip_func)oc_token_skip_run_cat1a, (oc_token_skip_func)oc_token_skip_run_cat1a, oc_token_skip_run,
	oc_token_skip_run, oc_token_skip_run, oc_token_skip_run};
		...
		for (; fragii < ncoded_fragis; fragii++)
		{
			...
			for (zzi = 0; zzi < 64;)
			{
				...
				if (eob_token[zzi] < OC_NDCT_EOB_TOKEN_MAX)
				{
					...
					frag_bits +=
						_enc->huff_codes[huffi][eob_token[zzi]].nbits + OC_DCT_TOKEN_EXTRA_BITS[eob_token[zzi]];
					eob_token[zzi] = OC_NDCT_EOB_TOKEN_MAX;
				}
			...
			oc_mode_metrics_add(OC_MODE_METRICS_SATD[modelines[qii][pli][qtj]][pli][qtj] + bin, fragw, satd,
								_enc->log_plq[qi][pli][qtj], frag_bits, sqrt_ssd);
			bin = OC_MINI(sad >> OC_SAD_SHIFT, OC_COMP_BINS - 1);
			oc_mode_metrics_add(OC_MODE_METRICS_SAD[modelines[qii][pli][qtj]][pli][qtj] + bin, fragw, sad,
								_enc->log_plq[qi][pli][qtj], frag_bits, sqrt_ssd);
		}
	}
	/*Update global SA(T)D/logq/rate/RMSE estimation matrix.*/
	oc_mode_metrics_update(OC_MODE_METRICS_SATD, 4, 1, OC_MODE_RD_SATD, OC_SATD_SHIFT, OC_MODE_RD_WEIGHT_SATD);
	oc_mode_metrics_update(OC_MODE_METRICS_SAD, 4, 1, OC_MODE_RD_SAD, OC_SAD_SHIFT, OC_MODE_RD_WEIGHT_SAD);
}
#endif
