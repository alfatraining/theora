...
static void oc_mcenc_find_candidates_a(oc_enc_ctx* _enc, oc_mcenc_ctx* _mcenc, oc_mv _accum, int _mbi, int _frame)
{
	oc_mb_enc_info* embs;
	int accum_x;
	int accum_y;
	...
	if (embs[_mbi].ncneighbors > 0)
	{
		...
		for (i = 0; i < embs[_mbi].ncneighbors; i++)
		{
			...
			_mcenc->candidates[ncandidates][0] = OC_MV_X(embs[nmbi].analysis_mv[0][_frame]);
			_mcenc->candidates[ncandidates][1] = OC_MV_Y(embs[nmbi].analysis_mv[0][_frame]);
			ncandidates++;
		}
	}
	...
	ncandidates++;
	_mcenc->candidates[ncandidates][0] = OC_CLAMPI(-31, OC_MV_X(embs[_mbi].analysis_mv[1][_frame]) + accum_x, 31);
	_mcenc->candidates[ncandidates][1] = OC_CLAMPI(-31, OC_MV_Y(embs[_mbi].analysis_mv[1][_frame]) + accum_y, 31);
	ncandidates++;
...
static void oc_mcenc_find_candidates_b(oc_enc_ctx* _enc, oc_mcenc_ctx* _mcenc, oc_mv _accum, int _mbi, int _frame)
{
	oc_mb_enc_info* embs;
	int accum_x;
	int accum_y;
	...
	_mcenc->candidates[ncandidates][0] = OC_CLAMPI(
		-31, 2 * OC_MV_X(embs[_mbi].analysis_mv[1][_frame]) - OC_MV_X(embs[_mbi].analysis_mv[2][_frame]) + accum_x, 31);
	_mcenc->candidates[ncandidates][1] = OC_CLAMPI(
		-31, 2 * OC_MV_Y(embs[_mbi].analysis_mv[1][_frame]) - OC_MV_Y(embs[_mbi].analysis_mv[2][_frame]) + accum_y, 31);
	ncandidates++;
	_mcenc->ncandidates = ncandidates;
}

static unsigned oc_sad16_halfpel(const oc_enc_ctx* _enc, const ptrdiff_t* _frag_buf_offs, const ptrdiff_t _fragis[4],
								 int _mvoffset0, int _mvoffset1, const unsigned char* _src, const unsigned char* _ref,
								 int _ystride, unsigned _best_err)
{
	unsigned err;
	int bi;
	err = 0;
...
static unsigned oc_satd16_halfpel(const oc_enc_ctx* _enc, const ptrdiff_t* _frag_buf_offs, const ptrdiff_t _fragis[4],
								  int _mvoffset0, int _mvoffset1, const unsigned char* _src, const unsigned char* _ref,
								  int _ystride, unsigned _best_err)
{
	unsigned err;
	...
	for (bi = 0; bi < 4; bi++)
	{
		ptrdiff_t frag_offs;
		frag_offs = _frag_buf_offs[_fragis[bi]];
		err += oc_enc_frag_satd2(_enc, &dc, _src + frag_offs, _ref + frag_offs + _mvoffset0,
								 _ref + frag_offs + _mvoffset1, _ystride);
		err += abs(dc);
	}
	return err;
}

static unsigned oc_mcenc_ysad_check_mbcandidate_fullpel(const oc_enc_ctx* _enc, const ptrdiff_t* _frag_buf_offs,
														const ptrdiff_t _fragis[4], int _dx, int _dy,
														const unsigned char* _src, const unsigned char* _ref,
														int _ystride, unsigned _block_err[4])
{
	unsigned err;
	int mvoffset;
	int bi;
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		unsigned block_err;
		frag_offs = _frag_buf_offs[_fragis[bi]];
		block_err = oc_enc_frag_sad(_enc, _src + frag_offs, _ref + frag_offs + mvoffset, _ystride);
...
static int oc_mcenc_ysatd_check_mbcandidate_fullpel(const oc_enc_ctx* _enc, const ptrdiff_t* _frag_buf_offs,
													const ptrdiff_t _fragis[4], int _dx, int _dy,
													const unsigned char* _src, const unsigned char* _ref, int _ystride)
{
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		{
			err += oc_enc_frag_satd(_enc, &dc, _src + frag_offs, _ref + frag_offs + mvoffset, _ystride);
			err += abs(dc);
		}
		else
		{
			err += oc_enc_frag_sad(_enc, _src + frag_offs, _ref + frag_offs + mvoffset, _ystride);
		}
	}
	return err;
}

static unsigned oc_mcenc_ysatd_check_bcandidate_fullpel(const oc_enc_ctx* _enc, ptrdiff_t _frag_offs, int _dx, int _dy,
														const unsigned char* _src, const unsigned char* _ref,
														int _ystride)
{
	unsigned err;
	int dc;
	err = oc_enc_frag_satd(_enc, &dc, _src + _frag_offs, _ref + _frag_offs + _dx + _dy * _ystride, _ystride);
	return err + abs(dc);
}
...
void oc_mcenc_search_frame(oc_enc_ctx* _enc, oc_mv _accum, int _mbi, int _frame, int _frame_full)
{
	...
	best_err = oc_mcenc_ysad_check_mbcandidate_fullpel(_enc, frag_buf_offs, fragis, candx, candy, src, ref, ystride,
													   block_err);
	best_vec[0] = candx;
	best_vec[1] = candy;
	...
	if (best_err > OC_YSAD_THRESH1)
	{
		...
		for (ci = 1; ci < mcenc.setb0; ci++)
		{
			...
			err = oc_mcenc_ysad_check_mbcandidate_fullpel(_enc, frag_buf_offs, fragis, candx, candy, src, ref, ystride,
														  block_err);
			if (err < best_err)
			{
				best_err = err;
		...
		if (best_err > t2)
		{
			...
			for (; ci < mcenc.ncandidates; ci++)
			{
				...
				if (hit_cache[candy + 15] & hitbit)
					continue;
				hit_cache[candy + 15] |= hitbit;
				err = oc_mcenc_ysad_check_mbcandidate_fullpel(_enc, frag_buf_offs, fragis, candx, candy, src, ref,
															  ystride, block_err);
				if (err < best_err)
				{
					best_err = err;
			...
			if (best_err > t2)
			{
				...
				for (;;)
				{
					...
					b = OC_DIV16(-best_vec[0] + 1) | OC_DIV16(best_vec[0] + 1) << 1 | OC_DIV16(-best_vec[1] + 1) << 2 |
					...
					for (sitei = 0; sitei < nsites; sitei++)
					{
						...
						if (hit_cache[candy + 15] & hitbit)
							continue;
						hit_cache[candy + 15] |= hitbit;
						err = oc_mcenc_ysad_check_mbcandidate_fullpel(_enc, frag_buf_offs, fragis, candx, candy, src,
																	  ref, ystride, block_err);
						if (err < best_err)
						{
							best_err = err;
							best_site = site;
					...
					for (bi = 0; bi < 4; bi++)
					{
						if (best_block_err[bi] > t2)
						{
							...
							for (;;)
							{
								...
								/*Compose the bit flags for boundary conditions.*/
								b = OC_DIV16(-bestx + 1) | OC_DIV16(bestx + 1) << 1 | OC_DIV16(-besty + 1) << 2 |
									OC_DIV16(besty + 1) << 3;
								nsites = OC_SQUARE_NSITES[b];
								for (sitei = 0; sitei < nsites; sitei++)
								{
									...
									if (hit_cache[candy + 15] & hitbit)
										continue;
									hit_cache[candy + 15] |= hitbit;
									err = oc_mcenc_ysad_check_mbcandidate_fullpel(_enc, frag_buf_offs, fragis, candx,
																				  candy, src, ref, ystride, block_err);
									if (err < best_err)
									{
										best_err = err;
										best_vec[0] = candx;
	...
	candx = best_vec[0];
	candy = best_vec[1];
	embs[_mbi].satd[_frame] =
		oc_mcenc_ysatd_check_mbcandidate_fullpel(_enc, frag_buf_offs, fragis, candx, candy, src, satd_ref, ystride);
	...
	if (_frame == OC_FRAME_PREV && _enc->sp_level < OC_SP_LEVEL_FAST_ANALYSIS)
	{
		for (bi = 0; bi < 4; bi++)
		{
			...
			embs[_mbi].block_satd[bi] = oc_mcenc_ysatd_check_bcandidate_fullpel(_enc, frag_buf_offs[fragis[bi]], candx,
																				candy, src, satd_ref, ystride);
...
static unsigned oc_mcenc_ysatd_halfpel_mbrefine(const oc_enc_ctx* _enc, int _mbi, int _vec[2], unsigned _best_err,
												int _frame)
{
	const unsigned char* src;
	const unsigned char* ref;
	...
	for (sitei = 0; sitei < 8; sitei++)
	{
		...
		if (_enc->sp_level < OC_SP_LEVEL_NOSATD)
		{
			err = oc_satd16_halfpel(_enc, frag_buf_offs, fragis, mvoffset0, mvoffset1, src, ref, ystride, _best_err);
		}
		else
		{
			err = oc_sad16_halfpel(_enc, frag_buf_offs, fragis, mvoffset0, mvoffset1, src, ref, ystride, _best_err);
		}
		if (err < _best_err)
		{
			_best_err = err;
...
void oc_mcenc_refine1mv(oc_enc_ctx* _enc, int _mbi, int _frame)
{
	...
	embs[_mbi].satd[_frame] = oc_mcenc_ysatd_halfpel_mbrefine(_enc, _mbi, vec, embs[_mbi].satd[_frame], _frame);
...
static unsigned oc_mcenc_ysatd_halfpel_brefine(const oc_enc_ctx* _enc, int _vec[2], const unsigned char* _src,
											   const unsigned char* _ref, int _ystride, int _offset_y[9],
											   unsigned _best_err)
{
	int mvoffset_base;
	int best_site;
	...
	for (sitei = 0; sitei < 8; sitei++)
	{
		...
		mvoffset0 = mvoffset_base + (dx & xmask) + (_offset_y[site] & ymask);
		mvoffset1 = mvoffset_base + (dx & ~xmask) + (_offset_y[site] & ~ymask);
		err = oc_enc_frag_satd2(_enc, &dc, _src, _ref + mvoffset0, _ref + mvoffset1, _ystride);
		err += abs(dc);
		if (err < _best_err)
		{
			_best_err = err;
...
void oc_mcenc_refine4mv(oc_enc_ctx* _enc, int _mbi)
{
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		embs[_mbi].block_satd[bi] = oc_mcenc_ysatd_halfpel_brefine(_enc, vec, src + frag_offs, ref + frag_offs, ystride,
																   offset_y, embs[_mbi].block_satd[bi]);
