	...
	for (qti = 0; qti < 2; qti++)
		for (pli = 0; pli < 3; pli++)
		{
			...
			for (qi = 0, qri = 0; qri <= _qinfo->qi_ranges[qti][pli].nranges; qri++)
			{
				...
				int qi_start;
				int qi_end;
				memcpy(base, _qinfo->qi_ranges[qti][pli].base_matrices[qri], sizeof(base));
				qi_start = qi;
				...
				for (;;)
				{
					  ...
					  This simple check helps us improve cache coherency later.*/
					{
						...
						for (qtj = 0; qtj <= qti; qtj++)
						{
							...
							{
								if (!memcmp(_dequant[qi][pli][qti], _dequant[qi][plj][qtj], sizeof(oc_quant_table)))
								{
									dupe = 1;
									break;
								}
					...
					{
						base[ci] =
							(unsigned char)((2 * ((qi_end - qi) * _qinfo->qi_ranges[qti][pli].base_matrices[qri][ci] +
												  (qi - qi_start) *
													  _qinfo->qi_ranges[qti][pli].base_matrices[qri + 1][ci]) +
