...
static unsigned short OC_ROUGH_TAN_LOOKUP[18] = {0,	358,  722,  1098, 1491, 1910,  2365,  2868,  3437,
...
static int oc_enc_find_qi_for_target(oc_enc_ctx* _enc, int _qti, int _qi_old, int _qi_min, ogg_int64_t _log_qtarget)
{
	...
	for (qi = _qi_min + 1; qi < 64; qi++)
	{
		...
		if (qdiff < best_qdiff || qdiff == best_qdiff && abs(qi - _qi_old) < abs(best_qi - _qi_old))
		{
			best_qi = qi;
			best_qdiff = qdiff;
		}
...
void oc_enc_calc_lambda(oc_enc_ctx* _enc, int _qti)
{
	...
	nqis = 1;
	if (lq < (OC_Q57(56) >> 3) && !_enc->vp3_compatible && _enc->sp_level < OC_SP_LEVEL_FAST_ANALYSIS)
	{
		qi1 = oc_enc_find_qi_for_target(_enc, _qti, OC_MAXI(qi - 1, 0), 0, lq + (OC_Q57(7) + 5) / 10);
		if (qi1 != qi)
			_enc->state.qis[nqis++] = qi1;
		qi1 = oc_enc_find_qi_for_target(_enc, _qti, OC_MINI(qi + 1, 63), 0, lq - (OC_Q57(6) + 5) / 10);
...
static void oc_enc_rc_reset(oc_enc_ctx* _enc)
{
	...
	_enc->rc.bits_per_frame = (_enc->state.info.target_bitrate * (ogg_int64_t)_enc->state.info.fps_denominator) /
	...
	/*Start with a buffer fullness of 50% plus 25% of the amount we plan to spend
	  ...
	  We can require fully half the bits in an interval for a keyframe, so this
	   ...
	   subsequent frames.*/
	_enc->rc.target = (_enc->rc.max + 1 >> 1) +
					  (_enc->rc.bits_per_frame + 2 >> 2) * OC_MINI(_enc->keyframe_frequency_force, _enc->rc.buf_delay);
	_enc->rc.fullness = _enc->rc.target;
	/*Pick exponents and initial scales for quantizer selection.*/
	npixels = _enc->state.info.frame_width * (ogg_int64_t)_enc->state.info.frame_height;
	...
	/*Set up second order followers, initialized according to corresponding
	   time constants.*/
	oc_iir_filter_init(&_enc->rc.scalefilter[0], 4, oc_q57_to_q24(_enc->rc.log_scale[0]));
	inter_delay = (_enc->rc.twopass ? OC_MAXI(_enc->keyframe_frequency_force, 12) : _enc->rc.buf_delay) >> 1;
	...
	oc_iir_filter_init(&_enc->rc.scalefilter[1], _enc->rc.inter_delay, oc_q57_to_q24(_enc->rc.log_scale[1]));
	oc_iir_filter_init(&_enc->rc.vfrfilter, 4, oc_bexp64_q24(_enc->rc.log_drop_scale));
}
...
void oc_rc_state_init(oc_rc_state* _rc, oc_enc_ctx* _enc)
{
	...
	if (_enc->state.info.target_bitrate > 0)
	{
		...
		_rc->buf_delay = _enc->keyframe_frequency_force > 256 ? 256 : _enc->keyframe_frequency_force;
...
void oc_rc_state_clear(oc_rc_state* _rc) { _ogg_free(_rc->frame_metrics); }
...
void oc_enc_rc_resize(oc_enc_ctx* _enc)
{
	...
	else
	{
		...
		{
			oc_iir_filter_init(&_enc->rc.scalefilter[1], idt, _enc->rc.scalefilter[1].y[0]);
	...
	if (_enc->rc.twopass == 2)
	{
		...
		if (cfm < buf_delay && (_enc->rc.frame_metrics != NULL || reset_window))
		{
			...
			int nfm;
			int fmh;
			fm = (oc_frame_metrics*)_ogg_realloc(_enc->rc.frame_metrics, buf_delay * sizeof(*_enc->rc.frame_metrics));
			if (fm == NULL)
			{
				...
				_enc->rc.buf_delay = _enc->rc.frame_metrics != NULL ? cfm : _enc->rc.frames_total[0] +
		...
		if (reset_window)
		{
			...
			_enc->rc.scale_window_end = _enc->rc.scale_window0 = _enc->state.curframe_num + _enc->prev_dup_count + 1;
...
int oc_enc_select_qi(oc_enc_ctx* _enc, int _qti, int _clamp)
{
	...
	switch (_enc->rc.twopass)
	{
		default:
		{
			...
			next_key_frame =
				_qti ? _enc->keyframe_frequency_force - (_enc->state.curframe_num - _enc->state.keyframe_num) : 0;
		...
		case 2:
		{
			...
			/*The window size can be slightly larger than the buffer window for VFR
			   ...
			   frames).*/
			buf_delay = OC_MINI(_enc->rc.scale_window_end - _enc->rc.scale_window0, _enc->rc.buf_delay);
			...
			buf_pad = OC_MINI(_enc->rc.buf_delay,
							  _enc->state.keyframe_num + _enc->keyframe_frequency_force - _enc->rc.scale_window0);
			...
			{
				ogg_int64_t scale;
				scale = _enc->rc.log_scale[_qti] < OC_Q57(23) ? oc_bexp64(_enc->rc.log_scale[_qti] + OC_Q57(24))
															  : 0x7FFFFFFFFFFFLL;
				scale *= nframes[_qti];
				nframes[_qti]++;
			...
			if (buf_pad > 0)
			{
				...
				/*And blend in the low-pass filtered scale according to how many frames
				   we added.*/
				scale = oc_bexp64(_enc->rc.log_scale[1] + OC_Q57(24)) * (ogg_int64_t)nframes[1] +
	...
	rate_bias = (_enc->rc.rate_bias / (_enc->state.curframe_num + 1000)) * (buf_delay - buf_pad);
	...
	else
	{
		...
		log_qtarget = OC_Q57(2) - ((oc_blog64(rlo) - log_scale0 + (_enc->rc.exp[_qti] >> 1)) / _enc->rc.exp[_qti] << 6);
	...
	if (_enc->rc.cap_overflow)
	{
		...
		if (log_scale0 - log_qexp < log_soft_limit)
		{
			...
			log_qexp += (log_scale0 - log_soft_limit - log_qexp >> 32) * ((OC_MINI(margin, soft_limit) << 32) / margin);
	...
	qi = oc_enc_find_qi_for_target(_enc, _qti, old_qi, _enc->state.info.quality, log_qtarget);
...
int oc_enc_update_rc_state(oc_enc_ctx* _enc, long _bits, int _qti, int _qi, int _trial, int _droppable)
{
	...
	if (_bits > 0)
	{
		...
		else
		{
			...
			if (_enc->rc.inter_delay < _enc->rc.inter_delay_target && _enc->rc.inter_count >= _enc->rc.inter_delay &&
				_qti == OC_INTER_FRAME)
			{
			...
			_enc->rc.log_scale[_qti] = oc_iir_filter_update(_enc->rc.scalefilter + _qti, oc_q57_to_q24(log_scale))
									   << 33;
			...
			else
			{
				...
				if (drop_count > 0x7F)
					...
					drop_count <<= 24;
				_enc->rc.log_drop_scale = oc_blog64(oc_iir_filter_update(&_enc->rc.vfrfilter, drop_count)) - OC_Q57(24);
...
int oc_enc_rc_2pass_out(oc_enc_ctx* _enc, unsigned char** _buf)
{
	if (_enc->rc.twopass_buffer_bytes == 0)
	{
		if (_enc->rc.twopass == 0)
		{
			...
			_enc->state.qis[0] = qi;
			_enc->rc.twopass = 1;
			_enc->rc.frames_total[0] = _enc->rc.frames_total[1] = _enc->rc.frames_total[2] = 0;
			_enc->rc.scale_sum[0] = _enc->rc.scale_sum[1] = 0;
		...
		else
		{
			...
			oc_rc_buffer_val(&_enc->rc, _enc->rc.cur_metrics.dup_count | _enc->rc.cur_metrics.frame_type << 31, 4);
			oc_rc_buffer_val(&_enc->rc, _enc->rc.cur_metrics.log_scale, 4);
			oc_rc_buffer_val(&_enc->rc, _enc->rc.cur_metrics.activity_avg, 4);
		}
	}
	else if (_enc->packet_state == OC_PACKET_DONE && _enc->rc.twopass_buffer_bytes != OC_RC_2PASS_HDR_SZ)
	{
		_enc->rc.twopass_buffer_bytes = 0;
		oc_rc_buffer_val(&_enc->rc, 0x5032544F, 4);
		oc_rc_buffer_val(&_enc->rc, OC_RC_2PASS_VERSION, 4);
...
static size_t oc_rc_buffer_fill(oc_rc_state* _rc, unsigned char* _buf, size_t _bytes, size_t _consumed, size_t _goal)
{
...
int oc_enc_rc_2pass_in(oc_enc_ctx* _enc, unsigned char* _buf, size_t _bytes)
{
	...
	if (_enc->rc.frames_total[0] == 0)
	{
		...
		consumed = oc_rc_buffer_fill(&_enc->rc, _buf, _bytes, consumed, OC_RC_2PASS_HDR_SZ);
		if (_enc->rc.twopass_buffer_fill >= OC_RC_2PASS_HDR_SZ)
		{
			...
			buf_delay = _enc->rc.frames_total[0] + _enc->rc.frames_total[1] + _enc->rc.frames_total[2];
			if (_enc->rc.frames_total[0] == 0 || buf_delay < 0 || (ogg_uint32_t)buf_delay < _enc->rc.frames_total[0] ||
	...
	if (_enc->rc.frames_total[0] != 0)
	{
		...
		else if (_enc->rc.twopass_buffer_bytes == 0)
		{
			if (_enc->rc.frame_metrics == NULL)
			{
				...
				consumed = oc_rc_buffer_fill(&_enc->rc, _buf, _bytes, consumed, OC_RC_2PASS_PACKET_SZ);
			...
			else
			{
				...
				frames_needed = OC_CLAMPI(0, _enc->rc.buf_delay - (_enc->rc.scale_window_end - _enc->rc.scale_window0),
										  _enc->rc.frames_left[0] + _enc->rc.frames_left[1] - _enc->rc.nframes[0] -
											  _enc->rc.nframes[1]);
				while (frames_needed > 0)
				{
					...
					if (_enc->rc.twopass_buffer_fill >= OC_RC_2PASS_PACKET_SZ)
					{
						...
						frames_needed =
							OC_CLAMPI(0, _enc->rc.buf_delay - (_enc->rc.scale_window_end - _enc->rc.scale_window0),
									  _enc->rc.frames_left[0] + _enc->rc.frames_left[1] - _enc->rc.nframes[0] -
										  _enc->rc.nframes[1]);
						/*Clear the buffer for the next frame.*/
						_enc->rc.twopass_buffer_fill = 0;
				...
				{
					int arg;
					*&_enc->rc.cur_metrics = *(_enc->rc.frame_metrics + _enc->rc.frame_metrics_head);
					_enc->rc.twopass_force_kf = _enc->rc.cur_metrics.frame_type == OC_INTRA_FRAME;
