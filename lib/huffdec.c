...
static int oc_huff_subtree_tokens(unsigned char _tokens[][2], int _depth)
{
	...
	do
	{
		if (_tokens[ti][1] - _depth < 32)
			code += 0x80000000U >> _tokens[ti++][1] - _depth;
		else
		{
...
static int oc_huff_tree_collapse_depth(unsigned char _tokens[][2], int _ntokens, int _depth)
{
	int got_leaves;
	int loccupancy;
	int occupancy;
	...
	do
	{
		...
		for (occupancy = ti = 0; ti < _ntokens; occupancy++)
		{
			...
			else if (_tokens[ti][1] == _depth + nbits)
			{
				...
				ti += oc_huff_subtree_tokens(_tokens + ti, _depth + nbits);
		}
...
static size_t oc_huff_node_size(int _nbits) { return 1 + (1 << _nbits); }
...
static size_t oc_huff_tree_collapse(ogg_int16_t* _tree, unsigned char _tokens[][2], int _ntokens)
{
	ogg_int16_t node[34];
	...
	do
	{
		...
		if (_tree != NULL)
			_tree[node[l]++] = (ogg_int16_t)nbits;
		do
		{
			...
			if (ti <= last[l])
			{
				...
				if (_tree != NULL)
					_tree[node[l]++] = (ogg_int16_t)ntree;
				l++;
				last[l] = (unsigned char)(ti + oc_huff_subtree_tokens(_tokens + ti, depth[l]) - 1);
				break;
			}
			/*Pop back up a level of recursion.*/
			else if (l-- > 0)
				nbits = depth[l + 1] - depth[l];
		} while (l >= 0);
	} while (l >= 0);
	return ntree;
}
...
/*Unpacks a set of Huffman trees, and reduces them to a collapsed
  ...
  Return: 0 on success, or a negative value on error.
		  The caller is responsible for cleaning up any partially initialized
		   _nodes on failure.*/
int oc_huff_trees_unpack(oc_pack_buf* _opb, ogg_int16_t* _nodes[TH_NHUFFMAN_TABLES])
{
	int i;
...
int oc_huff_trees_copy(ogg_int16_t* _dst[TH_NHUFFMAN_TABLES], const ogg_int16_t* const _src[TH_NHUFFMAN_TABLES])
{
	int total;
	int i;
	total = 0;
