...
static int oc_mode_scheme_chooser_scheme_mb_cost(const oc_mode_scheme_chooser* _chooser, int _scheme, int _mb_mode)
{
	int codebook;
	int ri;
	...
	if (_scheme == 0)
	{
		...
		while (ri > 0 && mc >= _chooser->mode_counts[_chooser->scheme0_list[ri - 1]])
			ri--;
	}
...
static int oc_mode_scheme_chooser_cost(oc_mode_scheme_chooser* _chooser, int _mb_mode)
{
	int scheme0;
	int scheme1;
	int best_bits;
	...
	{
		int cur_bits;
		cur_bits = scheme1_bits + oc_mode_scheme_chooser_scheme_mb_cost(_chooser, scheme1, _mb_mode);
...
static void oc_mode_scheme_chooser_update(oc_mode_scheme_chooser* _chooser, int _mb_mode)
{
	int ri;
	int si;
	...
	for (si = 0; si < 8; si++)
	{
		_chooser->scheme_bits[si] += OC_MODE_BITS[si + 1 >> 3][_chooser->mode_ranks[si][_mb_mode]];
	}
...
static int oc_block_run_bits(int _run_count) { return OC_BLOCK_RUN_CODE_NBITS[_run_count - 1]; }
...
static int oc_fr_state_sb_cost(const oc_fr_state* _fr, int _sb_partial, int _sb_full)
{
	int bits;
...
static void oc_fr_state_advance_sb(oc_fr_state* _fr, int _sb_partial, int _sb_full)
{
	int sb_partial_count;
	int sb_full_count;
	sb_partial_count = _fr->sb_partial_count;
	if (_fr->sb_partial != _sb_partial || sb_partial_count >= 4129)
		sb_partial_count = 0;
	sb_partial_count++;
	if (!_sb_partial)
	{
		sb_full_count = _fr->sb_full_count;
		if (_fr->sb_full != _sb_full || sb_full_count >= 4129)
			sb_full_count = 0;
		sb_full_count++;
...
static void oc_fr_skip_block(oc_fr_state* _fr) { oc_fr_state_advance_block(_fr, 0); }

static void oc_fr_code_block(oc_fr_state* _fr) { oc_fr_state_advance_block(_fr, 1); }
...
static void oc_qii_state_advance(oc_qii_state* _qd, const oc_qii_state* _qs, int _qii)
{
	ptrdiff_t bits;
	int qi01;
	int qi01_count;
...
static void oc_enc_pipeline_init(oc_enc_ctx* _enc, oc_enc_pipeline_state* _pipe)
{
	...
	for (pli = 0; pli < 3; pli++)
	{
		for (qii = 0; qii < nqis; qii++)
		{
			...
			for (qti = 0; qti < 2; qti++)
			{
				...
				_enc->state.dequant_tables[qi][pli][qti][0] = _enc->dequant_dc[qi0][pli][qti];
	...
	if (flimit != 0)
		oc_loop_filter_init(&_enc->state, _pipe->bounding_values, flimit);
...
static void oc_enc_pipeline_finish_mcu_plane(oc_enc_ctx* _enc, oc_enc_pipeline_state* _pipe, int _pli, int _sdelay,
											 int _edelay)
{
	...
	if (_pipe->nuncoded_fragis[_pli] > 0)
	{
		...
		oc_frag_copy_list(&_enc->state, _enc->state.ref_frame_data[OC_FRAME_SELF],
						  _enc->state.ref_frame_data[OC_FRAME_PREV], _enc->state.ref_ystride[_pli],
						  _pipe->uncoded_fragis[_pli], _pipe->nuncoded_fragis[_pli], _enc->state.frag_buf_offs);
		_pipe->nuncoded_fragis[_pli] = 0;
	}
	/*Perform DC prediction.*/
	oc_enc_pred_dc_frag_rows(_enc, _pli, _pipe->fragy0[_pli], _pipe->fragy_end[_pli]);
	/*Finish DC tokenization.*/
	oc_enc_tokenize_dc_frag_list(_enc, _pli, _pipe->coded_fragis[_pli], _pipe->ncoded_fragis[_pli],
	...
	{
		oc_state_loop_filter_frag_rows(&_enc->state, _pipe->bounding_values, OC_FRAME_SELF, _pli,
	...
	oc_state_borders_fill_rows(&_enc->state, _enc->state.ref_frame_idx[OC_FRAME_SELF], _pli,
...
static int oc_enc_block_transform_quantize(oc_enc_ctx* _enc, oc_enc_pipeline_state* _pipe, int _pli, ptrdiff_t _fragi,
										   unsigned _rd_scale, unsigned _rd_iscale, oc_rd_metric* _mo, oc_fr_state* _fr,
										   oc_token_checkpoint** _stack)
{
	...
	switch (mb_mode)
	{
		...
		default:
		{
			...
			nmv_offs = oc_state_get_mv_offsets(&_enc->state, mv_offs, _pli, frag_mvs[_fragi]);
			if (nmv_offs > 1)
			{
				oc_enc_frag_copy2(_enc, dst, ref + mv_offs[0], ref + mv_offs[1], ystride);
	...
	{
		ac_bits = oc_enc_tokenize_ac(_enc, _pli, _fragi, idct, data, dequant, dct, nonzero + 1, _stack,
									 OC_RD_ISCALE(_enc->lambda, _rd_iscale), qti ? 0 : 3);
	}
	else
	{
		ac_bits = oc_enc_tokenize_ac_fast(_enc, _pli, _fragi, idct, data, dequant, dct, nonzero + 1, _stack,
										  OC_RD_ISCALE(_enc->lambda, _rd_iscale), qti ? 0 : 3);
	}
	/*Reconstruct.
	...
	{
		oc_enc_frag_recon_inter(_enc, dst, nmv_offs == 1 ? ref + mv_offs[0] : dst, ystride, data);
	}
/*If _fr is NULL, then this is an INTRA frame, and we can't skip blocks.*/
...
#endif
	{
		...
		{
			coded_ssd = oc_enc_frag_border_ssd(_enc, src, dst, ystride, _enc->state.borders[borderi].mask);
		}
...
static int oc_enc_mb_transform_quantize_inter_luma(oc_enc_ctx* _enc, oc_enc_pipeline_state* _pipe, unsigned _mbi,
												   int _mode_overhead, const unsigned _rd_scale[4],
												   const unsigned _rd_iscale[4])
{
	...
	*&qs_checkpoint = *(_pipe->qs + 0);
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		if (oc_enc_block_transform_quantize(_enc, _pipe, 0, fragi, _rd_scale[bi], _rd_iscale[bi], &mo, _pipe->fr + 0,
											&stackptr))
		{
	...
	if (ncoded > 0 && !mo.dc_flag)
	{
		...
		cost =
			mo.coded_ac_ssd + _enc->lambda * (mo.ac_bits + oc_fr_cost4(&fr_checkpoint, _pipe->fr + 0) + _mode_overhead);
...
static void oc_enc_sb_transform_quantize_inter_chroma(oc_enc_ctx* _enc, oc_enc_pipeline_state* _pipe, int _pli,
													  int _sbi_start, int _sbi_end)
{
		...
		for (quadi = 0; quadi < 4; quadi++)
			for (bi = 0; bi < 4; bi++)
			{
				...
				if (fragi >= 0)
				{
					...
					stackptr = stack;
					if (oc_enc_block_transform_quantize(_enc, _pipe, _pli, fragi, rd_scale, rd_iscale, &mo, fr,
														&stackptr))
					{
						coded_fragis[ncoded_fragis++] = fragi;
					}
	...
	for (qii = 0; qii < _enc->state.nqis; qii++)
	{
		...
		for (pli = 0; pli < 3; pli++)
		{
			...
			for (qti = 0; qti < 2; qti++)
			{
				...
				for (modeline = 0; modeline < OC_LOGQ_BINS - 1 && OC_MODE_LOGQ[modeline + 1][pli][qti] > log_plq;
					 modeline++)
					;
...
/*Estimate the R-D cost of the DCT coefficients given the SATD of a block after
   prediction.*/
static unsigned oc_dct_cost2(oc_enc_ctx* _enc, unsigned* _ssd, int _qii, int _pli, int _qti, int _satd)
{
	unsigned rmse;
	int shift;
	int bin;
...
static unsigned oc_mb_activity(oc_enc_ctx* _enc, unsigned _mbi, unsigned _activity[4])
{
	const unsigned char* src;
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		else
		{
			...
			for (i = 0; i < 8; i++)
			{
				for (j = 0; j < 8; j++)
				{
					e1 += abs((s[j + 2] - s[j] << 1) + (s - ystride)[j + 2] - (s - ystride)[j] + (s + ystride)[j + 2] -
							  (s + ystride)[j]);
					e2 += abs(((s + ystride)[j + 1] - (s - ystride)[j + 1] << 1) + (s + ystride)[j] - (s - ystride)[j] +
							  (s + ystride)[j + 2] - (s - ystride)[j + 2]);
					e3 += abs(((s + ystride)[j + 2] - (s - ystride)[j] << 1) + (s + ystride)[j + 1] - s[j] + s[j + 2] -
							  (s - ystride)[j + 1]);
					e4 += abs(((s + ystride)[j] - (s - ystride)[j + 2] << 1) + (s + ystride)[j + 1] - s[j + 2] + s[j] -
							  (s - ystride)[j + 1]);
				}
				s += ystride;
			}
...
static void oc_mb_activity_fast(oc_enc_ctx* _enc, unsigned _mbi, unsigned _activity[4], const unsigned _intra_satd[12])
{
	int bi;
	for (bi = 0; bi < 4; bi++)
	{
...
static unsigned oc_mb_masking(unsigned _rd_scale[5], unsigned _rd_iscale[5], const ogg_uint16_t _chroma_rd_scale[2],
							  const unsigned _activity[4], unsigned _activity_avg, unsigned _luma, unsigned _luma_avg)
{
...
/*The ratio lb/la is meant to approximate
  ...
  lb=4*_luma+_luma_avg;
	...
	for (bi = 2; bi < 4; bi++)
	{
		...
		else if (_rd_iscale[bi] < _rd_iscale[bi_min2])
			bi_min2 = bi;
	}
	...
	if (_rd_iscale[bi_min] < (1 << OC_RD_ISCALE_BITS))
		bi_min = bi_min2;
...
static int oc_mb_intra_satd(oc_enc_ctx* _enc, unsigned _mbi, unsigned _frag_satd[12])
{
	const unsigned char* src;
	const ptrdiff_t* frag_buf_offs;
	const ptrdiff_t* sb_map;
...
static unsigned oc_analyze_intra_mb_luma(oc_enc_ctx* _enc, const oc_qii_state* _qs, unsigned _mbi,
										 const unsigned _rd_scale[4])
{
	...
	for (qii = 0; qii < nqis; qii++)
	{
		...
		rate[0][qii] =
			oc_dct_cost2(_enc, ssd[0] + qii, qii, 0, 0, satd) + (qs[0][qii].bits - _qs->bits << OC_BIT_SCALE);
...
static unsigned oc_analyze_intra_chroma_block(oc_enc_ctx* _enc, const oc_qii_state* _qs, int _pli, ptrdiff_t _fragi,
											  unsigned _rd_scale)
{
	const unsigned char* src;
	oc_fragment* frags;
...
/*Most chroma blocks have no AC coefficients to speak of anyway, so it's not
  ...
  nqis=_enc->state.nqis;
	...
	for (qii = 0; qii < nqis; qii++)
	{
		...
		cur_rate = oc_dct_cost2(_enc, &cur_ssd, qii, _pli, 0, satd) + (qt[qii].bits - _qs->bits << OC_BIT_SCALE);
...
static void oc_enc_mb_transform_quantize_intra_luma(oc_enc_ctx* _enc, oc_enc_pipeline_state* _pipe, unsigned _mbi,
													const unsigned _rd_scale[4], const unsigned _rd_iscale[4])
{
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		oc_enc_block_transform_quantize(_enc, _pipe, 0, fragi, _rd_scale[bi], _rd_iscale[bi], NULL, NULL, &stackptr);
		coded_fragis[ncoded_fragis++] = fragi;
	}
	_pipe->ncoded_fragis[0] = ncoded_fragis;
}

static void oc_enc_sb_transform_quantize_intra_chroma(oc_enc_ctx* _enc, oc_enc_pipeline_state* _pipe, int _pli,
													  int _sbi_start, int _sbi_end)
{
	const ogg_uint16_t* mcu_rd_scale;
	const ogg_uint16_t* mcu_rd_iscale;
		...
		for (quadi = 0; quadi < 4; quadi++)
			for (bi = 0; bi < 4; bi++)
			{
				...
				if (fragi >= 0)
				{
					...
					stackptr = stack;
					oc_enc_block_transform_quantize(_enc, _pipe, _pli, fragi, rd_scale, rd_iscale, NULL, NULL,
													&stackptr);
					coded_fragis[ncoded_fragis++] = fragi;
				}
			}
...
void oc_enc_analyze_intra(oc_enc_ctx* _enc, int _recode)
{
	...
	for (stripe_sby = 0; notdone; stripe_sby += mcu_nvsbs)
	{
			...
			for (quadi = 0; quadi < 4; quadi++)
				if (sb_flags[sbi].quad_valid & 1 << quadi)
				{
					...
					else
					{
						...
						for (bi = 0; bi < 4; bi++)
							frags[sb_maps[mbi >> 2][mbi & 3][bi]].qii = 0;
					}
					activity_sum +=
						oc_mb_masking(rd_scale, rd_iscale, chroma_rd_scale, activity, activity_avg, luma, luma_avg);
					luma_sum += luma;
					/*Motion estimation:
					  We do a basic 1MV search for all macroblocks, coded or not,
					   keyframe or not, unless we aren't using motion estimation at all.*/
					if (!_recode && _enc->state.curframe_num > 0 && _enc->sp_level < OC_SP_LEVEL_NOMC &&
					...
					oc_enc_mb_transform_quantize_intra_luma(_enc, &_enc->pipe, mbi, rd_scale, rd_iscale);
		...
		for (pli = 1; pli < 3; pli++)
		{
			oc_enc_sb_transform_quantize_intra_chroma(_enc, &_enc->pipe, pli, _enc->pipe.sbi0[pli],
													  _enc->pipe.sbi_end[pli]);
	...
	_enc->activity_avg = OC_MAXI(OC_ACTIVITY_AVG_MIN, (unsigned)((activity_sum + (_enc->state.fplanes[0].nfrags >> 1)) /
...
{
	_modec->cost = OC_MODE_RD_COST(_modec->ssd, _modec->rate + _modec->overhead, _lambda);
}

/*A set of skip SSD's to use to disable early skipping.*/
static const unsigned OC_NOSKIP[12] = {UINT_MAX, UINT_MAX, UINT_MAX, UINT_MAX, UINT_MAX, UINT_MAX,
...
static void oc_analyze_mb_mode_luma(oc_enc_ctx* _enc, oc_mode_choice* _modec, const oc_fr_state* _fr,
									const oc_qii_state* _qs, const unsigned _frag_satd[12],
									const unsigned _skip_ssd[12], const unsigned _rd_scale[4], int _qti)
{
	oc_fr_state fr;
	oc_qii_state qs;
	unsigned ssd;
	...
	*&qs = *_qs;
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		*(ft + 0) = *&fr;
		...
		best_rate = oc_dct_cost2(_enc, &best_ssd, 0, 0, _qti, satd) + (cur_overhead << OC_BIT_SCALE);
		if (nqis > 1)
		{
...
static void oc_analyze_mb_mode_chroma(oc_enc_ctx* _enc, oc_mode_choice* _modec, const oc_fr_state* _fr,
									  const oc_qii_state* _qs, const unsigned _frag_satd[12],
									  const unsigned _skip_ssd[12], unsigned _rd_scale, int _qti)
{
	unsigned ssd;
	unsigned rate;
	unsigned satd;
  ...
  nqis=_enc->state.nqis;
	...
	for (pli = 1; pli < 3; pli++)
	{
		for (; bi < nblocks; bi++)
		{
			unsigned best_cost;
			satd = _frag_satd[bi];
			best_rate = oc_dct_cost2(_enc, &best_ssd, 0, pli, _qti, satd) + OC_CHROMA_QII_RATE;
			best_ssd = OC_RD_SCALE(best_ssd, _rd_scale);
			best_cost = OC_MODE_RD_COST(ssd + best_ssd, rate + best_rate, lambda);
			best_qii = 0;
			for (qii = 1; qii < nqis; qii++)
			{
				cur_rate = oc_dct_cost2(_enc, &cur_ssd, qii, pli, _qti, satd) + OC_CHROMA_QII_RATE;
				cur_ssd = OC_RD_SCALE(cur_ssd, _rd_scale);
				cur_cost = OC_MODE_RD_COST(ssd + cur_ssd, rate + cur_rate, lambda);
				if (cur_cost < best_cost)
				{
...
static void oc_skip_cost(oc_enc_ctx* _enc, oc_enc_pipeline_state* _pipe, unsigned _mbi, const unsigned _rd_scale[4],
						 unsigned _ssd[12])
{
	const unsigned char* src;
	const unsigned char* ref;
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		{
			uncoded_ssd = oc_enc_frag_border_ssd(_enc, src + frag_offs, ref + frag_offs, ystride,
...
static void oc_cost_intra(oc_enc_ctx* _enc, oc_mode_choice* _modec, unsigned _mbi, const oc_fr_state* _fr,
						  const oc_qii_state* _qs, const unsigned _frag_satd[12], const unsigned _skip_ssd[12],
						  const unsigned _rd_scale[5])
{
	oc_analyze_mb_mode_luma(_enc, _modec, _fr, _qs, _frag_satd, _skip_ssd, _rd_scale, 0);
	oc_analyze_mb_mode_chroma(_enc, _modec, _fr, _qs, _frag_satd, _skip_ssd, _rd_scale[4], 0);
	_modec->overhead = oc_mode_scheme_chooser_cost(&_enc->chooser, OC_MODE_INTRA) << OC_BIT_SCALE;
	oc_mode_set_cost(_modec, _enc->lambda);
}

static void oc_cost_inter(oc_enc_ctx* _enc, oc_mode_choice* _modec, unsigned _mbi, int _mb_mode, oc_mv _mv,
						  ...
						  const unsigned _rd_scale[5])
{
	...
	if (oc_state_get_mv_offsets(&_enc->state, mv_offs, 0, _mv) > 1)
	{
		for (bi = 0; bi < 4; bi++)
		{
			...
			if (_enc->sp_level < OC_SP_LEVEL_NOSATD)
			{
				frag_satd[bi] = oc_enc_frag_satd2(_enc, &dc, src + frag_offs, ref + frag_offs + mv_offs[0],
												  ref + frag_offs + mv_offs[1], ystride);
				frag_satd[bi] += abs(dc);
			}
			else
			{
				frag_satd[bi] = oc_enc_frag_sad2_thresh(_enc, src + frag_offs, ref + frag_offs + mv_offs[0],
	...
	else
	{
		for (bi = 0; bi < 4; bi++)
		{
			...
			if (_enc->sp_level < OC_SP_LEVEL_NOSATD)
			{
				frag_satd[bi] = oc_enc_frag_satd(_enc, &dc, src + frag_offs, ref + frag_offs + mv_offs[0], ystride);
				frag_satd[bi] += abs(dc);
			}
			else
			{
				frag_satd[bi] = oc_enc_frag_sad(_enc, src + frag_offs, ref + frag_offs + mv_offs[0], ystride);
			}
		}
	}
	mb_map = (const oc_mb_map_plane*)_enc->state.mb_maps[_mbi];
	...
	if (oc_state_get_mv_offsets(&_enc->state, mv_offs, 1, _mv) > 1)
	{
		for (mapii = 4; mapii < map_nidxs; mapii++)
		{
			...
			if (_enc->sp_level < OC_SP_LEVEL_NOSATD)
			{
				frag_satd[mapii] = oc_enc_frag_satd2(_enc, &dc, src + frag_offs, ref + frag_offs + mv_offs[0],
													 ref + frag_offs + mv_offs[1], ystride);
				frag_satd[mapii] += abs(dc);
			}
			else
			{
				frag_satd[mapii] = oc_enc_frag_sad2_thresh(_enc, src + frag_offs, ref + frag_offs + mv_offs[0],
														   ref + frag_offs + mv_offs[1], ystride, UINT_MAX);
			}
		}
	}
	else
	{
		for (mapii = 4; mapii < map_nidxs; mapii++)
		{
			...
			if (_enc->sp_level < OC_SP_LEVEL_NOSATD)
			{
				frag_satd[mapii] = oc_enc_frag_satd(_enc, &dc, src + frag_offs, ref + frag_offs + mv_offs[0], ystride);
				frag_satd[mapii] += abs(dc);
			}
			else
			{
				frag_satd[mapii] = oc_enc_frag_sad(_enc, src + frag_offs, ref + frag_offs + mv_offs[0], ystride);
			}
		}
	}
	oc_analyze_mb_mode_luma(_enc, _modec, _fr, _qs, frag_satd, _skip_ssd, _rd_scale, 1);
	oc_analyze_mb_mode_chroma(_enc, _modec, _fr, _qs, frag_satd, _skip_ssd, _rd_scale[4], 1);
	_modec->overhead = oc_mode_scheme_chooser_cost(&_enc->chooser, _mb_mode) << OC_BIT_SCALE;
	oc_mode_set_cost(_modec, _enc->lambda);
}

static void oc_cost_inter_nomv(oc_enc_ctx* _enc, oc_mode_choice* _modec, unsigned _mbi, int _mb_mode,
							   const oc_fr_state* _fr, const oc_qii_state* _qs, const unsigned _skip_ssd[12],
							   const unsigned _rd_scale[4])
{
...
static int oc_cost_inter1mv(oc_enc_ctx* _enc, oc_mode_choice* _modec, unsigned _mbi, int _mb_mode, oc_mv _mv,
							const oc_fr_state* _fr, const oc_qii_state* _qs, const unsigned _skip_ssd[12],
							const unsigned _rd_scale[4])
{
	int bits0;
	...
	_modec->overhead +=
		OC_MINI(_enc->mv_bits[0] + bits0, _enc->mv_bits[1] + 12) - OC_MINI(_enc->mv_bits[0], _enc->mv_bits[1])
		<< OC_BIT_SCALE;
	oc_mode_set_cost(_modec, _enc->lambda);
	return bits0;
}
...
static const unsigned char OC_MB_PHASE[4][4] = {{0, 1, 3, 2}, {0, 3, 1, 2}, {0, 3, 1, 2}, {2, 3, 1, 0}};

static void oc_cost_inter4mv(oc_enc_ctx* _enc, oc_mode_choice* _modec, unsigned _mbi, oc_mv _mv[4],
							 const oc_fr_state* _fr, const oc_qii_state* _qs, const unsigned _skip_ssd[12],
							 const unsigned _rd_scale[5])
{
	unsigned frag_satd[12];
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		{
			satd = oc_enc_frag_satd2(_enc, &dc, src + frag_offs, ref + frag_offs + mv_offs[0],
									 ref + frag_offs + mv_offs[1], ystride);
		}
		else
		{
			satd = oc_enc_frag_satd(_enc, &dc, src + frag_offs, ref + frag_offs + mv_offs[0], ystride);
		}
		frag_satd[OC_MB_PHASE[_mbi & 3][bi]] = satd + abs(dc);
	}
	oc_analyze_mb_mode_luma(_enc, _modec, _fr, _qs, frag_satd, _enc->vp3_compatible ? OC_NOSKIP : _skip_ssd, _rd_scale,
							1);
	...
	for (bi = 0; bi < 4; bi++)
	{
		...
		{
			lbmvs[bi] = _mv[bi];
			bits0 += OC_MV_BITS[0][OC_MV_X(_mv[bi]) + 31] + OC_MV_BITS[0][OC_MV_Y(_mv[bi]) + 31];
			bits1 += 12;
		}
	}
	...
	oc_analyze_mb_mode_chroma(_enc, _modec, _fr, _qs, frag_satd, _skip_ssd, _rd_scale[4], 1);
	_modec->overhead = oc_mode_scheme_chooser_cost(&_enc->chooser, OC_MODE_INTER_MV_FOUR) +
...
int oc_enc_analyze_inter(oc_enc_ctx* _enc, int _allow_keyframe, int _recode)
{
	...
	for (stripe_sby = 0; notdone; stripe_sby += mcu_nvsbs)
	{
				...
				if (sb_flags[sbi].quad_valid & 1 << quadi)
				{
					...
					/*Motion estimation:
					  We always do a basic 1MV search for all macroblocks, coded or not,
					   keyframe or not.*/
					if (!_recode && sp_level < OC_SP_LEVEL_NOMC)
						oc_mcenc_search(_enc, mbi);
					mv = 0;
					...
					if (_allow_keyframe)
					{
						oc_cost_intra(_enc, modes + OC_MODE_INTRA, mbi, _enc->pipe.fr + 0, &intra_luma_qs, intra_satd,
									  OC_NOSKIP, rd_scale);
						intrabits += modes[OC_MODE_INTRA].rate;
						for (bi = 0; bi < 4; bi++)
						{
							oc_qii_state_advance(&intra_luma_qs, &intra_luma_qs, modes[OC_MODE_INTRA].qii[bi]);
						}
					}
					...
					{
						oc_cost_inter_nomv(_enc, modes + OC_MODE_INTER_NOMV, mbi, OC_MODE_INTER_NOMV, _enc->pipe.fr + 0,
										   _enc->pipe.qs + 0, skip_ssd, rd_scale);
						oc_cost_intra(_enc, modes + OC_MODE_INTRA, mbi, _enc->pipe.fr + 0, _enc->pipe.qs + 0,
									  intra_satd, skip_ssd, rd_scale);
						mb_mv_bits_0 = oc_cost_inter1mv(_enc, modes + OC_MODE_INTER_MV, mbi, OC_MODE_INTER_MV,
														embs[mbi].unref_mv[OC_FRAME_PREV], _enc->pipe.fr + 0,
														_enc->pipe.qs + 0, skip_ssd, rd_scale);
						oc_cost_inter(_enc, modes + OC_MODE_INTER_MV_LAST, mbi, OC_MODE_INTER_MV_LAST, last_mv,
									  _enc->pipe.fr + 0, _enc->pipe.qs + 0, skip_ssd, rd_scale);
						oc_cost_inter(_enc, modes + OC_MODE_INTER_MV_LAST2, mbi, OC_MODE_INTER_MV_LAST2, prior_mv,
									  _enc->pipe.fr + 0, _enc->pipe.qs + 0, skip_ssd, rd_scale);
						oc_cost_inter_nomv(_enc, modes + OC_MODE_GOLDEN_NOMV, mbi, OC_MODE_GOLDEN_NOMV,
										   _enc->pipe.fr + 0, _enc->pipe.qs + 0, skip_ssd, rd_scale);
						mb_gmv_bits_0 = oc_cost_inter1mv(_enc, modes + OC_MODE_GOLDEN_MV, mbi, OC_MODE_GOLDEN_MV,
														 embs[mbi].unref_mv[OC_FRAME_GOLD], _enc->pipe.fr + 0,
														 _enc->pipe.qs + 0, skip_ssd, rd_scale);
						...
						if (sp_level < OC_SP_LEVEL_FAST_ANALYSIS)
						{
							oc_cost_inter4mv(_enc, modes + OC_MODE_INTER_MV_FOUR, mbi, embs[mbi].block_mv,
											 _enc->pipe.fr + 0, _enc->pipe.qs + 0, skip_ssd, rd_scale);
						}
						else
						{
							...
							modes[OC_MODE_INTER_MV_FOUR].cost < modes[OC_MODE_GOLDEN_MV].cost)
						{
							...
							oc_cost_inter4mv(_enc, modes + OC_MODE_INTER_MV_FOUR, mbi, embs[mbi].ref_mv,
											 _enc->pipe.fr + 0, _enc->pipe.qs + 0, skip_ssd, rd_scale);
						}
						else if (modes[OC_MODE_GOLDEN_MV].cost + inter_mv_pref < modes[OC_MODE_INTER_MV].cost)
						{
							...
							mb_gmv_bits_0 = oc_cost_inter1mv(_enc, modes + OC_MODE_GOLDEN_MV, mbi, OC_MODE_GOLDEN_MV,
															 embs[mbi].analysis_mv[0][OC_FRAME_GOLD], _enc->pipe.fr + 0,
															 _enc->pipe.qs + 0, skip_ssd, rd_scale);
						}
						...
						mb_mv_bits_0 = oc_cost_inter1mv(_enc, modes + OC_MODE_INTER_MV, mbi, OC_MODE_INTER_MV,
														embs[mbi].analysis_mv[0][OC_FRAME_PREV], _enc->pipe.fr + 0,
														_enc->pipe.qs + 0, skip_ssd, rd_scale);
					...
					if (oc_enc_mb_transform_quantize_inter_luma(
							_enc, &_enc->pipe, mbi, modes[mb_mode].overhead >> OC_BIT_SCALE, rd_scale, rd_iscale) > 0)
					{
						int orig_mb_mode;
						...
						switch (mb_mode)
						{
							case OC_MODE_INTER_MV:
							{
								...
								if (orig_mb_mode == OC_MODE_INTER_MV_FOUR)
								{
									...
									mb_mv_bits_0 = OC_MV_BITS[0][OC_MV_X(mv) + 31] + OC_MV_BITS[0][OC_MV_Y(mv) + 31];
								}
								/*Otherwise we used the original analysis MV.*/
								else
									last_mv = embs[mbi].analysis_mv[0][OC_FRAME_PREV];
							...
							case OC_MODE_INTER_MV_FOUR:
							{
								...
								for (bi = 0; bi < 4; bi++)
								{
									...
									if (frags[fragi].coded)
									{
										...
										_enc->mv_bits[0] +=
											OC_MV_BITS[0][OC_MV_X(last_mv) + 31] + OC_MV_BITS[0][OC_MV_Y(last_mv) + 31];
										_enc->mv_bits[1] += 12;
									}
		...
		for (pli = 1; pli < 3; pli++)
		{
			oc_enc_sb_transform_quantize_inter_chroma(_enc, &_enc->pipe, pli, _enc->pipe.sbi0[pli],
													  _enc->pipe.sbi_end[pli]);
			oc_enc_pipeline_finish_mcu_plane(_enc, &_enc->pipe, pli, notstart, notdone);
		}
		notstart = 1;
	...
	if (_allow_keyframe)
	{
		...
		for (pli = 1; pli < 3; pli++)
			interbits += _enc->pipe.fr[pli].bits << OC_BIT_SCALE;
