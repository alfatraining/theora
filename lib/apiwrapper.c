...
const char* theora_version_string(void) { return th_version_string(); }

ogg_uint32_t theora_version_number(void) { return th_version_number(); }

void theora_info_init(theora_info* _ci) { memset(_ci, 0, sizeof(*_ci)); }
...
int theora_control(theora_state* _th, int _req, void* _buf, size_t _buf_sz)
{
	...
	if (_th->internal_decode != NULL)
	{
		return (*((oc_state_dispatch_vtable*)_th->internal_decode)->control)(_th, _req, _buf, _buf_sz);
	}
	else if (_th->internal_encode != NULL)
	{
		return (*((oc_state_dispatch_vtable*)_th->internal_encode)->control)(_th, _req, _buf, _buf_sz);
	}
	else
		return TH_EINVAL;
}
...
ogg_int64_t theora_granule_frame(theora_state* _th, ogg_int64_t _gp)
{
	...
	if (_th->internal_decode != NULL)
	{
		return (*((oc_state_dispatch_vtable*)_th->internal_decode)->granule_frame)(_th, _gp);
	}
	else if (_th->internal_encode != NULL)
	{
		return (*((oc_state_dispatch_vtable*)_th->internal_encode)->granule_frame)(_th, _gp);
	}
	else
		return -1;
}
...
double theora_granule_time(theora_state* _th, ogg_int64_t _gp)
{
	...
	if (_th->internal_decode != NULL)
	{
		return (*((oc_state_dispatch_vtable*)_th->internal_decode)->granule_time)(_th, _gp);
	}
	else if (_th->internal_encode != NULL)
	{
		return (*((oc_state_dispatch_vtable*)_th->internal_encode)->granule_time)(_th, _gp);
	}
	else
		return -1;
}
...
int theora_packet_isheader(ogg_packet* _op) { return th_packet_isheader(_op); }
...
void theora_comment_init(theora_comment* _tc) { th_comment_init((th_comment*)_tc); }
...
void theora_comment_add(theora_comment* _tc, char* _comment) { th_comment_add((th_comment*)_tc, _comment); }
