...
const th_huff_code TH_VP31_HUFF_CODES[TH_NHUFFMAN_TABLES][TH_NDCT_TOKENS] = {{{0x002D, 6},
																			  {0x0026, 7},
																			  {0x0166, 9},
																			  {0x004E, 8},
																			  {0x02CE, 10},
	...
	for (i = 0; i < TH_NHUFFMAN_TABLES; i++)
	{
		...
		for (j = 0; j < TH_NDCT_TOKENS; j++)
		{
			...
			if (j + 1 < TH_NDCT_TOKENS)
			{
				...
				if (!(entries[j + 1].pattern & bit) || (entries[j].pattern & mask) != (entries[j + 1].pattern & mask))
				{
					return TH_EINVAL;
				}
			}
