...
static int theora_encode_control(theora_state* _te, int _req, void* _buf, size_t _buf_sz)
{
	return th_encode_ctl(((th_api_wrapper*)_te->i->codec_setup)->encode, _req, _buf, _buf_sz);
}

static ogg_int64_t theora_encode_granule_frame(theora_state* _te, ogg_int64_t _gp)
{
...
static const oc_state_dispatch_vtable OC_ENC_DISPATCH_VTBL = {
	(oc_state_clear_func)theora_encode_clear, (oc_state_control_func)theora_encode_control,
	(oc_state_granule_frame_func)theora_encode_granule_frame, (oc_state_granule_time_func)theora_encode_granule_time,
};
...
int theora_encode_init(theora_state* _te, theora_info* _ci)
{
	...
	*&apiinfo->info = *_ci;
	...
	keyframe_frequency_force = _ci->keyframe_auto_p ? _ci->keyframe_frequency_force : _ci->keyframe_frequency;
	th_encode_ctl(apiinfo->api.encode, TH_ENCCTL_SET_KEYFRAME_FREQUENCY_FORCE, &keyframe_frequency_force,
...
int theora_encode_comment(theora_comment* _tc, ogg_packet* _op)
{
	...
	ret = oc_state_flushheader(NULL, &packet_state, &opb, NULL, NULL, th_version_string(), (th_comment*)_tc, _op);
	if (ret >= 0)
	{
