...
void oc_dec_dc_unpredict_mcu_plane_c64x(oc_dec_ctx* _dec, oc_dec_pipeline_state* _pipe, int _pli)
{
				...
				if (frags[fragi].coded)
				{
					...
					else
					{
						pred = (_dotp2(_pack2(p0, p1), OC_PRED_SCALE[pflags][0]) +
								_dotp2(_pack2(p2, p3), OC_PRED_SCALE[pflags][1])) /
							   128;
						if ((pflags & 7) == 7)
						{
							if (abs(pred - p1) > 128)
								pred = p1;
							else if (abs(pred - p3) > 128)
								pred = p3;
