...
/*14 cycles.*/
void oc_frag_copy_c64x(unsigned char* restrict _dst, const unsigned char* restrict _src, int _ystride)
{
...
/*34 cycles.*/
void oc_frag_recon_intra_c64x(unsigned char* _dst, int _ystride, const ogg_int16_t _residue[64])
{
	int i;
	for (i = 0; i < 8; i++)
	{
							  ...
							  int _last_zzi, ogg_uint16_t _dc_quant)
{
	...
	else
	{
		...
		{
			oc_frag_recon_inter2_c64x(dst, ref + mvoffsets[0], ref + mvoffsets[1], ystride, _dct_coeffs + 64);
		}
		else
			oc_frag_recon_inter_c64x(dst, ref + mvoffsets[0], ystride, _dct_coeffs + 64);
	}
}
	...
	while (fragi0 < fragi0_end)
	{
		...
		while (fragi < fragi_end)
		{
			if (frags[fragi].coded)
			{
				...
				if (fragi > fragi0)
					loop_filter_h(ref, ystride, ll);
				if (fragi0 > fragi_top)
					loop_filter_v(ref, ystride, ll);
