...
static const unsigned char OC_FZIG_ZAG_NEON[128] = {
	...
	41, 48, 56, 49, 42, 35, 28, 21, 14, 7,  15, 22, 29, 36, 43, 50, 57, 58, 51, 44, 37, 30, 23, 31, 38, 45,
	52, 59, 60, 53, 46, 39, 47, 54, 61, 62, 55, 63, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64,
...
void oc_state_accel_init_arm(oc_theora_state* _state)
{
	...
	_state->opt_vtable.state_loop_filter_frag_rows = (oc_state_loop_filter_frag_rows_func)oc_loop_filter_frag_rows_arm;
#endif
							 ...
							 int _last_zzi, ogg_uint16_t _dc_quant)
{
	...
	else
	{
		...
		{
			oc_frag_recon_inter2_arm(dst, ref + mvoffsets[0], ref + mvoffsets[1], ystride, _dct_coeffs + 64);
		}
		else
			oc_frag_recon_inter_arm(dst, ref + mvoffsets[0], ystride, _dct_coeffs + 64);
	}
}
							...
							int _last_zzi, ogg_uint16_t _dc_quant)
{
	...
	else
	{
		...
		if (oc_state_get_mv_offsets(_state, mvoffsets, _pli, _state->frag_mvs[_fragi]) > 1)
		{
			oc_frag_recon_inter2_v6(dst, ref + mvoffsets[0], ref + mvoffsets[1], ystride, _dct_coeffs + 64);
		}
		else
			oc_frag_recon_inter_v6(dst, ref + mvoffsets[0], ystride, _dct_coeffs + 64);
	}
}
							  ...
							  int _last_zzi, ogg_uint16_t _dc_quant)
{
	...
	else
	{
		...
		if (oc_state_get_mv_offsets(_state, mvoffsets, _pli, _state->frag_mvs[_fragi]) > 1)
		{
			oc_frag_recon_inter2_neon(dst, ref + mvoffsets[0], ref + mvoffsets[1], ystride, _dct_coeffs + 64);
		}
		else
