...
/*x86-64 guarantees SIMD support up through at least SSE2.
  If the best routine we have available only needs SSE2 (which at the moment
   ...
   call.*/
#define oc_frag_copy(_state, _dst, _src, _ystride) oc_frag_copy_mmx(_dst, _src, _ystride)
#define oc_frag_copy_list(_state, _dst_frame, _src_frame, _ystride, _fragis, _nfragis, _frag_buf_offs)                 \
	oc_frag_copy_list_mmx(_dst_frame, _src_frame, _ystride, _fragis, _nfragis, _frag_buf_offs)
#define oc_frag_recon_intra(_state, _dst, _ystride, _residue) oc_frag_recon_intra_mmx(_dst, _ystride, _residue)
...
#define oc_idct8x8(_state, _y, _x, _last_zzi) oc_idct8x8_sse2(_y, _x, _last_zzi)
#define oc_state_frag_recon oc_state_frag_recon_mmx
#define oc_loop_filter_init(_state, _bv, _flimit) oc_loop_filter_init_mmxext(_bv, _flimit)
#define oc_state_loop_filter_frag_rows oc_state_loop_filter_frag_rows_mmxext
#define oc_restore_fpu(_state) oc_restore_fpu_mmx()
#else
...
#define OC_MEM_OFFS(_offs, _name) OC_M2STR(_offs - 8 + % H[_name])
#endif
...
#define OC_MEM_OFFS(_offs, _name) OC_M2STR(_offs + % [_name])
#endif
...
#define OC_ARRAY_OPERAND(_type, _ptr, _size)                                                                           \
	(*(                                                                                                                \
		...
		struct                                                                                                         \
		{                                                                                                              \
			_type array_value__[(_size)];                                                                              \
		}* array_addr__ = (void*)(_ptr);                                                                               \
		array_addr__;                                                                                                  \
		 }))
...
void oc_frag_copy_list_mmx(unsigned char* _dst_frame, const unsigned char* _src_frame, int _ystride,
...
void oc_frag_recon_inter2_mmx(unsigned char* _dst, const unsigned char* _src1, const unsigned char* _src2, int _ystride,
...
void oc_state_frag_recon_mmx(const oc_theora_state* _state, ptrdiff_t _fragi, int _pli, ogg_int16_t _dct_coeffs[128],
...
void oc_state_loop_filter_frag_rows_mmx(const oc_theora_state* _state, signed char _bv[256], int _refi, int _pli,
										int _fragy0, int _fragy_end);
void oc_state_loop_filter_frag_rows_mmxext(const oc_theora_state* _state, signed char _bv[256], int _refi, int _pli,
										   int _fragy0, int _fragy_end);
