...
{
	ptrdiff_t ret;
	__asm__ __volatile__("movq (%[ref1]),%%mm0\n\t"
						 "movq (%[ref2]),%%mm1\n\t"
						 "movq (%[ref1],%[ystride]),%%mm2\n\t"
						 "movq (%[ref2],%[ystride]),%%mm3\n\t"
						 "xor %[ret],%[ret]\n\t"
						 ...
						 "pcmpeqb %%mm6,%%mm6\n\t"
						 "movq (%[src],%[ystride]),%%mm5\n\t"
						 "psubb %%mm6,%%mm7\n\t" OC_SAD2_LOOP OC_SAD2_LOOP OC_SAD2_LOOP OC_SAD2_TAIL
...
#define OC_HADAMARD_C_ABS_ACCUM_8x4(_r6, _r7)                                                                          \
	OC_HADAMARD_C_ABS_ACCUM_A_8x4(_r6, _r7) OC_HADAMARD_C_ABS_ACCUM_B_8x4(_r6, _r7)
...
#define OC_HADAMARD_ABS_ACCUM_8x4(_r6, _r7) OC_HADAMARD_AB_8x4 OC_HADAMARD_C_ABS_ACCUM_8x4(_r6, _r7)
				 ...
				 buf) "\n\t"                                                                                           \
					  ...
					  "movq " OC_MEM_OFFS(                                                                             \
						  ...
						  buf) ",%%mm5\n\t" /*Second 4x4 transpose:*/ /*mm4 = a3 a2 a1 a0                              \
							   ...
							   "movq %%mm6,%%mm0\n\t"                                                                  \
							   "punpcklwd %%mm7,%%mm6\n\t"                                                             \
							   "movq %%mm1," OC_MEM_OFFS(                                                              \
								   0x50 + (_off),                                                                      \
								   buf) "\n\t"                                                                         \
										"punpckhwd %%mm7,%%mm0\n\t"                                                    \
										"movq %%mm4,%%mm7\n\t"                                                         \
										"punpcklwd %%mm5,%%mm4\n\t"                                                    \
										"movq %%mm2," OC_MEM_OFFS(                                                     \
											0x60 + (_off),                                                             \
											buf) "\n\t"                                                                \
												 "punpckhwd %%mm5,%%mm7\n\t" /*mm4 = b1 a1 b0 a0                       \
																			   mm7 = b3 a3 b2 a2                       \
...
static unsigned oc_int_frag_satd_mmxext(int* _dc, const unsigned char* _src, int _src_ystride,
										const unsigned char* _ref, int _ref_ystride)
{
	...
	int dc;
	__asm__ __volatile__(
		OC_LOAD_SUB_8x4(0x00) OC_HADAMARD_8x4 OC_TRANSPOSE_4x4x2(0x00)
		...
		"movq %%mm4," OC_MEM_OFFS(0x00, buf) "\n\t"
											 "movq %%mm5," OC_MEM_OFFS(
												 0x10, buf) "\n\t"
															"movq %%mm6," OC_MEM_OFFS(
																0x20, buf) "\n\t"
																		   "movq %%mm7," OC_MEM_OFFS(
																			   0x30, buf) "\n\t" OC_LOAD_SUB_8x4(0x04)
																			   OC_HADAMARD_8x4 OC_TRANSPOSE_4x4x2(0x08)
		...
		/*We split out the stages here so we can save the DC coefficient in the
		   middle.*/
		OC_HADAMARD_AB_8x4 OC_HADAMARD_C_ABS_ACCUM_A_8x4(
			0x28, 0x38) "movd %%mm1,%[dc]\n\t" OC_HADAMARD_C_ABS_ACCUM_B_8x4(0x28, 0x38)
													 ...
													 "movq " OC_MEM_OFFS(
														 ...
														 buf) ",%%mm7\n\t"
															  "movq " OC_MEM_OFFS(
																  0x40,
																  buf) ",%%mm0\n\t"
																	   "movq " OC_MEM_OFFS(0x48, buf) ",%%"
																									  "mm4\n"
																									  "\t" OC_HADAMARD_ABS_ACCUM_8x4(
																										  0x68,
																										  0x78) "pmaddw"
																												"d "
																												"%%mm7,"
		...
		: [ret] "=d"(ret), [ret2] "=r"(ret2), [dc] "=a"(dc), [buf] "=m"(OC_ARRAY_OPERAND(ogg_int16_t, buf, 64))
		: [src] "r"(_src), [src_ystride] "c"((ptrdiff_t)_src_ystride), [ref] "r"(_ref),
		  [ref_ystride] "d"((ptrdiff_t)_ref_ystride)
							  ...
							  const unsigned char* _src2, int _src_ystride)
{
	__asm__ __volatile__(
		...
		: [dst_ystride] "r"((ptrdiff_t)_dst_ystride), [src_ystride] "r"((ptrdiff_t)_src_ystride)
		: "memory");
}
...
unsigned oc_enc_frag_intra_satd_mmxext(int* _dc, const unsigned char* _src, int _ystride)
{
	...
	int dc;
	__asm__ __volatile__(
		OC_LOAD_8x4(0x00) OC_HADAMARD_8x4 OC_TRANSPOSE_4x4x2(0x00)
		/*Finish swapping out this 8x4 block to make room for the next one.
		  mm0...mm3 have been swapped out already.*/
		"movq %%mm4," OC_MEM_OFFS(0x00, buf) "\n\t"
											 "movq %%mm5," OC_MEM_OFFS(
		...
		: [ret] "=a"(ret), [ret2] "=r"(ret2), [dc] "=r"(dc), [buf] "=m"(OC_ARRAY_OPERAND(ogg_int16_t, buf, 64))
		: [src] "r"(_src), [src4] "r"(_src + 4 * _ystride), [ystride] "r"((ptrdiff_t)_ystride),
		  [ystride3] "r"((ptrdiff_t)3 * _ystride)
...
void oc_enc_frag_sub_mmx(ogg_int16_t _residue[64], const unsigned char* _src, const unsigned char* _ref, int _ystride)
{
	int i;
...
void oc_enc_frag_sub_128_mmx(ogg_int16_t _residue[64], const unsigned char* _src, int _ystride)
{
	ptrdiff_t ystride3;
	__asm__ __volatile__(
		/*mm0=[src]*/
