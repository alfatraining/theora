							 ...
							 int _last_zzi, ogg_uint16_t _dc_quant)
{
	...
	if (refi == OC_FRAME_SELF)
		oc_frag_recon_intra_mmx(dst, ystride, _dct_coeffs + 64);
	else
	{
		...
		{
			oc_frag_recon_inter2_mmx(dst, ref + mvoffsets[0], ref + mvoffsets[1], ystride, _dct_coeffs + 64);
		}
		else
			oc_frag_recon_inter_mmx(dst, ref + mvoffsets[0], ystride, _dct_coeffs + 64);
	}
}
...
void oc_loop_filter_init_mmx(signed char _bv[256], int _flimit) { memset(_bv, _flimit, 8); }
...
void oc_loop_filter_init_mmxext(signed char _bv[256], int _flimit) { memset(_bv, ~(_flimit << 1), 8); }
