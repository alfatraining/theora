						   ...
						   const ptrdiff_t* _fragis, ptrdiff_t _nfragis, const ptrdiff_t* _frag_buf_offs)
{
	...
	for (fragii = 0; fragii < _nfragis; fragii++)
	{
		...
		OC_FRAG_COPY_MMX(_dst_frame + frag_buf_off, _src_frame + frag_buf_off, _ystride);
	}
}
...
void oc_frag_recon_intra_mmx(unsigned char* _dst, int _ystride, const ogg_int16_t* _residue)
{
	__asm__ __volatile__(
		...
		:
		: [residue] "r"(_residue), [dst] "r"(_dst), [dst4] "r"(_dst + (_ystride << 2)),
		  [ystride] "r"((ptrdiff_t)_ystride), [ystride3] "r"((ptrdiff_t)_ystride * 3)
		: "memory");
}
							  ...
							  const ogg_int16_t* _residue)
{
	...
	for (i = 4; i-- > 0;)
	{
		__asm__ __volatile__(
			...
			/*Advance dest ptr.*/
			"lea (%[dst],%[ystride],2),%[dst]\n\t"
			: [dst] "+r"(_dst), [residue] "+r"(_residue), [src1] "+%r"(_src1), [src2] "+r"(_src2)
			: [ystride] "r"((ptrdiff_t)_ystride)
			: "memory");
	}
}

void oc_restore_fpu_mmx(void) { __asm__ __volatile__("emms\n\t"); }
#endif
