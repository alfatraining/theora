...
{
	unsigned ret;
	__asm__ __volatile__(OC_LOAD_SUB_4x8("%%xmm7") OC_SSD_4x8("%%xmm7") OC_LOAD_SUB_4x8("%%xmm0")
							 OC_SSD_4x8("%%xmm0") "paddd %%xmm0,%%xmm7\n\t"
												  "movdqa %%xmm7,%%xmm6\n\t"
...
static const unsigned char
	__attribute__((aligned(16))) OC_MASK_CONSTS[8] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80};
									 ...
									 ogg_int64_t _mask)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		if (m)
		{
			__asm__ __volatile__(OC_LOAD_SUB_MASK_2x8 "pmaddwd %%xmm0,%%xmm0\n\t"
													  "pmaddwd %%xmm1,%%xmm1\n\t"
													  "paddd %%xmm0,%%xmm7\n\t"
													  "paddd %%xmm1,%%xmm7\n\t"
	...
	__asm__ __volatile__("movdqa %%xmm7,%%xmm6\n\t"
						 "punpckhqdq %%xmm7,%%xmm7\n\t"
						 "paddd %%xmm6,%%xmm7\n\t"
						 "pshufd $1,%%xmm7,%%xmm6\n\t"
						 "paddd %%xmm6,%%xmm7\n\t"
...
#define OC_HADAMARD_8x8 OC_HADAMARD_AB_8x8 OC_HADAMARD_C_8x8
...
#define OC_HADAMARD_C_ABS_ACCUM_A_8x8                                                                                  \
	...
	"movdqa %%xmm7," OC_MEM_OFFS(                                                                                      \
		0x10, buf) "\n\t"                                                                                              \
				   "movdqa %%xmm6," OC_MEM_OFFS(                                                                       \
					   0x00, buf) "\n\t" /*xmm7={0x7FFF}x4                                                             \
...
#define OC_HADAMARD_C_ABS_ACCUM_8x8 OC_HADAMARD_C_ABS_ACCUM_A_8x8 OC_HADAMARD_C_ABS_ACCUM_B_8x8
...
#define OC_HADAMARD_ABS_ACCUM_8x8 OC_HADAMARD_AB_8x8 OC_HADAMARD_C_ABS_ACCUM_8x8

static unsigned oc_int_frag_satd_sse2(int* _dc, const unsigned char* _src, int _src_ystride, const unsigned char* _ref,
									  int _ref_ystride)
{
	...
	unsigned ret2;
	int dc;
	__asm__ __volatile__(OC_LOAD_SUB_8x8 OC_HADAMARD_8x8 OC_TRANSPOSE_8x8
						 /*We split out the stages here so we can save the DC coefficient in the
							middle.*/
						 OC_HADAMARD_AB_8x8 OC_HADAMARD_C_ABS_ACCUM_A_8x8
						 "movd %%xmm1,%[dc]\n\t" OC_HADAMARD_C_ABS_ACCUM_B_8x8
						 ...
						 : [src] "S"(_src), [src_ystride] "c"((ptrdiff_t)_src_ystride), [ref] "a"(_ref),
...
unsigned oc_enc_frag_intra_satd_sse2(int* _dc, const unsigned char* _src, int _ystride)
{
	...
	unsigned ret;
	int dc;
	__asm__ __volatile__(OC_LOAD_8x8 OC_HADAMARD_8x8 OC_TRANSPOSE_8x8
						 /*We split out the stages here so we can save the DC coefficient in the
							middle.*/
						 OC_HADAMARD_AB_8x8 OC_HADAMARD_C_ABS_ACCUM_A_8x8
						 "movd %%xmm1,%[dc]\n\t" OC_HADAMARD_C_ABS_ACCUM_B_8x8
						 ...
						 /*Although it looks like we're using 7 registers here, gcc can alias %[ret]
							and %[dc] with some of the inputs, since for once we don't write to
							them until after we're done using everything but %[buf].*/
						 : [ret] "=a"(ret), [dc] "=r"(dc), [buf] "=m"(OC_ARRAY_OPERAND(ogg_int16_t, buf, 16))
